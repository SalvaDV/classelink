...
// ─── TAB: CONFIGURACIÓN ───────────────────────────────────────────────────────
const CFG_DEFAULTS = { comision_pct: 10, max_publicaciones_docente: 20, verificacion_ia_activa: true, mp_activo: true, stripe_activo: true, admins: ADMIN_EMAILS.join(", ") };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card>
        ...
        <div style={{ fontSize: 13, color: C.muted, marginBottom: 12, lineHeight: 1.6 }}>
          Los emails listados acá tienen acceso al panel de administración. <strong>Importante:</strong> después de guardar, también actualizá la lista <code>ADMIN_EMAILS</code> en el código de <code>AdminPage.jsx</code> y <code>admin-actions/index.ts</code> — esas listas son la fuente de verdad de seguridad.
        </div>
        <CfgRow label="Emails admin" sub="Separados por coma">
          <input
            value={cfg.admins}
            onChange={e => set("admins", e.target.value)}
            style={{ width: 300, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 12px", color: C.text, fontSize: 13, outline: "none", fontFamily: FONT }} />
        </CfgRow>
      </Card>
