...
function Sidebar({page,setPage,session,onLogout,onNewPost,unreadCount,ofertasCount,notifCount,ofertasAceptadasNuevas,mobile,open,onClose}){
  ...
  const nav=[
    ...
    {id:"cuenta",icon:"○",label:"Mi cuenta",badge:ofertasAceptadasNuevas},
  ];
  const inner=(
...
function FavBtn({post,session,onFavChange}){
  ...
  return(
    <button onClick={toggle} title={favId?"Quitar favorito":"Agregar a favoritos"}
      style={{background:"none",border:"none",fontSize:18,cursor:"pointer",color:favId?C.accent:"#aaa",transition:"color .15s",padding:"0 3px",lineHeight:1,opacity:loading?0.5:1}}
  ...
  if(miOferta&&miOferta.estado==="pendiente"){
    ...
    return(
      <div style={{display:"flex",flexDirection:"column",gap:3,alignSelf:"center"}}>
        ...
        {diasEspera!==null&&<span style={{fontSize:10,color:C.muted}}>Enviada hace {diasEspera===0?"hoy":diasEspera===1?"1 día":`${diasEspera} días`}</span>}
      </div>
    );
  }
...
function PostCard({post,session,onOpenChat,onOpenDetail,onOpenPerfil,avgPub,countPub,avgUser,yaOferte,fueRechazado}){
  ...
  return(
    <div onClick={()=>onOpenDetail(post)} style={{background:fueRechazado?"#E05C5C08":C.card,border:`1px solid ${fueRechazado?"#E05C5C33":C.border}`,borderRadius:16,padding:"16px 18px",cursor:"pointer",transition:"border-color .2s,transform .15s",position:"relative",overflow:"hidden",fontFamily:FONT}}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=fueRechazado?"#E05C5C77":C.accent;e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor=fueRechazado?"#E05C5C33":C.border;e.currentTarget.style.transform="translateY(0)";}}>
      ...
      {yaOferte&&!esMio&&<div style={{position:"absolute",top:10,right:12,background:"#E0955C22",border:"1px solid #E0955C55",borderRadius:20,fontSize:9,fontWeight:700,color:C.warn,padding:"2px 8px",letterSpacing:.4}}>⏳ Oferta enviada</div>}
      {fueRechazado&&!esMio&&<div style={{position:"absolute",top:10,right:12,background:"#E05C5C15",border:"1px solid #E05C5C44",borderRadius:20,fontSize:9,fontWeight:700,color:C.danger,padding:"2px 8px",letterSpacing:.4}}>✗ Rechazada</div>}
...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil}){
  ...
  const cargar=useCallback(async()=>{
    setLoading(true);
    try{
      const [d,misOfertas]=await Promise.all([
  ...
  const clearAll=()=>{setFiltroTipo("all");setFiltroModo("all");setFiltroSinc("all");setFiltroMateria("");setSliderMin(precioMin);setSliderMax(precioMax);setFiltroFechaDesde("");setFiltroFechaHasta("");setFiltroDurMin(0);setBusqueda("");};
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{position:"relative",marginBottom:10}}>
        <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar por tema, materia o docente..." style={{width:"100%",background:C.surface,border:`1px solid ${busqueda?C.accent:C.border}`,borderRadius:11,padding:"10px 34px 10px 13px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:FONT}}/>
      ...
      <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
        {panelOpen&&(
          <div style={{width:210,flexShrink:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 14px",position:"sticky",top:16,maxHeight:"calc(100vh - 80px)",overflowY:"auto"}}>
            ...
            <div style={{marginBottom:14}}><FL ch="Materia"/><select value={filtroMateria} onChange={e=>setFiltroMateria(e.target.value)} style={selS}><option value="">Todas</option>{MATERIAS.map(m=><option key={m} value={m}>{m}</option>)}</select></div>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          {loading?<Spinner/>:filtered.length===0?(
            ...
            <div style={{display:"grid",gap:11}}>
              {filtered.map(p=><PostCard key={p.id} post={p} session={session} onOpenChat={onOpenChat} onOpenDetail={onOpenDetail} onOpenPerfil={onOpenPerfil} avgPub={reseñasMap[p.id]?.avg} countPub={reseñasMap[p.id]?.count} avgUser={reseñasUserMap[p.autor_email]} yaOferte={pendientesIds.has(p.id)} fueRechazado={rechazadasIds.has(p.id)}/>)}
            </div>
          )}
        </div>
      </div>
...
function ContraofertaModal({oferta,miRol,session,onClose,onEnviada}){
  ...
  const enviar=async()=>{
    if(!precio||!msg.trim())return;setSaving(true);
    try{
  ...
  return(
    <Modal onClose={onClose} width="min(420px,96vw)">
      <div style={{padding:"22px 24px"}}>
        ...
        <Btn onClick={enviar} disabled={saving||!precio||!msg.trim()} style={{width:"100%",padding:"10px"}}>{saving?"Enviando...":"Enviar contraoferta →"}</Btn>
      </div>
    </Modal>
  );
}
...
function InscripcionesPage({session,onOpenCurso,onOpenChat,onMarkNotifsRead}){
  ...
  const marcarNotifPubLeida=async(pubId)=>{
    ...
    try{
      await sb.marcarTodasNotifsLeidas(session.user.email,session.access_token);
        ...
        {tipo==="oferta"&&(<>
          ...
          {modo==="curso"&&(<div style={{borderTop:`1px solid ${C.border}`,paddingTop:10}}>
            ...
            {sinc==="sinc"&&(<>
              ...
              {clasesSinc.map((c,i)=>(<div key={i} style={{display:"flex",gap:5,alignItems:"center",marginBottom:6,background:C.card,borderRadius:9,padding:"7px 9px",border:`1px solid ${c.hora_fin<=c.hora_inicio?"#E05C5C44":C.border}`}}>
                ...
                <input type="time" value={c.hora_inicio} onChange={e=>{const v=e.target.value;updClase(i,"hora_inicio",v);if(c.hora_fin&&c.hora_fin<=v){const[h,m]=v.split(":").map(Number);const fin=`${String(h+(m>=30?1:0)).padStart(2,"0")}:${m>=30?"00":String(m+30).padStart(2,"0")}`;updClase(i,"hora_fin",fin);}}} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,padding:"4px 7px",color:C.text,fontSize:11,fontFamily:FONT,outline:"none",colorScheme:"dark",flex:2}}/>
                <span style={{color:C.muted,fontSize:11}}>→</span>
                <input type="time" value={c.hora_fin} onChange={e=>{const v=e.target.value;if(v<=c.hora_inicio)return;updClase(i,"hora_fin",v);}} style={{background:C.surface,border:`1px solid ${c.hora_fin<=c.hora_inicio?C.danger:C.border}`,borderRadius:7,padding:"4px 7px",color:c.hora_fin<=c.hora_inicio?C.danger:C.text,fontSize:11,fontFamily:FONT,outline:"none",colorScheme:"dark",flex:2}}/>
...
function DocenteStats({pubs,reseñas,inscritosMap}){
  ...
  const stats=[
    ...
    {label:"Alumnos totales",val:totalAlumnos,color:C.info},
    {label:"Reseñas",val:reseñas.length,color:C.accent},
    {label:"Rating promedio",val:avg?`${avg.toFixed(1)}★`:"—",color:C.warn},
    {label:"Finalizadas",val:finalizadas.length,color:C.muted},
  ];
  if(ofertas.length===0&&totalAlumnos===0)return null;
  return(
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px 20px",marginBottom:20}}>
      <div style={{fontWeight:700,color:C.text,fontSize:14,marginBottom:12}}>Estadísticas de docente</div>
...
function ContraRespondedor({oferta,session,onActualizado}){
  const [modo,setModo]=useState(null);// null | "contra"
  const [precio,setPrecio]=useState("");const [tipo,setTipo]=useState(oferta.precio_tipo||"hora");const [msg,setMsg]=useState("");const [saving,setSaving]=useState(false);
  const aceptar=async()=>{
    setSaving(true);
    try{
      // Aceptar al precio de la contraoferta del alumno
  ...
  const enviarContra=async()=>{
    ...
    try{
      ...
      setModo(null);onActualizado();
    }catch(e){alert(e.message);}finally{setSaving(false);}
  };
  const iS={background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 9px",color:C.text,fontSize:11,outline:"none",fontFamily:FONT};
  if(modo==="contra") return(
    <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0,minWidth:130}}>
      <div style={{display:"flex",gap:4}}>
        <input value={precio} onChange={e=>setPrecio(e.target.value)} placeholder="Precio" type="number" min="0" style={{...iS,flex:2,width:0}}/>
        <select value={tipo} onChange={e=>setTipo(e.target.value)} style={{...iS,flex:1,cursor:"pointer",colorScheme:"dark"}}>
          <option value="hora">/h</option><option value="clase">/cl</option><option value="mes">/mes</option>
        </select>
      </div>
      <textarea value={msg} onChange={e=>setMsg(e.target.value)} placeholder="Mensaje..." style={{...iS,width:"100%",minHeight:50,resize:"vertical",boxSizing:"border-box"}}/>
      <div style={{display:"flex",gap:4}}>
        <button onClick={enviarContra} disabled={saving||!precio||!msg.trim()} style={{flex:1,background:C.accent,border:"none",borderRadius:7,color:"#0D0D0D",padding:"5px 0",cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:FONT,opacity:saving?0.5:1}}>Enviar</button>
        <button onClick={()=>setModo(null)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:7,color:C.muted,padding:"5px 8px",cursor:"pointer",fontSize:10,fontFamily:FONT}}>×</button>
      </div>
    </div>
  );
  return(
    <div style={{display:"flex",flexDirection:"column",gap:4,flexShrink:0}}>
      <div style={{fontSize:10,fontWeight:700,color:"#C85CE0",background:"#C85CE015",border:"1px solid #C85CE033",borderRadius:7,padding:"3px 8px",textAlign:"center"}}>↔ Contraoferta</div>
      <div style={{fontSize:11,color:C.accent,fontWeight:600,textAlign:"center"}}>{fmtPrice(oferta.contraoferta_precio)} /{oferta.contraoferta_tipo||oferta.precio_tipo}</div>
      <button onClick={aceptar} disabled={saving} style={{background:"#4ECB7122",border:"1px solid #4ECB7144",borderRadius:7,color:C.success,padding:"4px 8px",cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:FONT}}>Aceptar precio</button>
      <button onClick={()=>setModo("contra")} disabled={saving} style={{background:"#C85CE015",border:"1px solid #C85CE033",borderRadius:7,color:"#C85CE0",padding:"4px 8px",cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:FONT}}>Volver a ofertar</button>
      <button onClick={rechazar} disabled={saving} style={{background:"#E05C5C15",border:"1px solid #E05C5C33",borderRadius:7,color:C.danger,padding:"4px 8px",cursor:"pointer",fontSize:10,fontFamily:FONT}}>Rechazar</button>
    </div>
  );
}
...
function MiCuentaPage({session,onOpenDetail,onOpenCurso,onEdit,onNew,onOpenChat,onRefreshOfertas,onClearBadge}){
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {/* ── Búsquedas ── */}
      {(()=>{
        // Estado local para confirm de eliminación de búsqueda
        const BusquedasList=()=>{
          const [confirmBusq,setConfirmBusq]=useState(null);// {p, ofertanteAcept}
      ...
      {(()=>{
        ...
        // IDs de aceptaciones ya vistas (localStorage)
        const vistasKey="ofertasAceptadasVistas_"+email;
        let vistas=[];try{vistas=JSON.parse(localStorage.getItem(vistasKey)||"[]");}catch{}
        const marcarVista=(id)=>{if(!vistas.includes(id)){const nv=[...vistas,id];localStorage.setItem(vistasKey,JSON.stringify(nv));}};
        return(
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>{
                const esNuevaAceptacion=o.estado==="aceptada"&&!vistas.includes(o.id);
                const diasEspera=o.estado==="pendiente"&&o.created_at?Math.floor((Date.now()-new Date(o.created_at))/86400000):null;
                return(
                <div key={o.id} style={{background:esNuevaAceptacion?"#4ECB7108":C.surface,border:`1px solid ${esNuevaAceptacion?"#4ECB71":o.estado==="rechazada"?"#E05C5C33":o.estado==="aceptada"?"#4ECB7133":C.border}`,borderRadius:12,padding:"11px 14px",display:"flex",gap:10,alignItems:"center",position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{flex:1,minWidth:0,paddingLeft:6}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}>
                      ...
                      {/* Idea creativa: indicador de tiempo de espera para pendientes */}
                  ...
                  {/* Contraoferta pendiente del alumno hacia el docente */}
                  {o.estado==="pendiente"&&o.contraoferta_precio&&o.contraoferta_de==="alumno"&&(
                    <ContraRespondedor oferta={o} session={session} onActualizado={cargar}/>
                  )}
                  ...
                  {o.estado==="aceptada"&&(
                    <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0}} onClick={()=>marcarVista(o.id)}>
                      <button onClick={()=>{marcarVista(o.id);setEspacioModal({...o,_rol:"docente"});}} style={{background:C.accent,border:"none",borderRadius:8,color:"#0D0D0D",padding:"6px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>Ver espacio →</button>
                      <button onClick={()=>{marcarVista(o.id);setAcuerdoModal({...o,_rol:"docente"});}} style={{background:"#4ECB7115",border:"1px solid #4ECB7133",borderRadius:8,color:C.success,padding:"5px 10px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>{o.acuerdo_confirmado?"✓ Ver acuerdo":"📋 Acuerdo"}</button>
                      <button onClick={async()=>{marcarVista(o.id);try{await sb.updateOfertaBusq(o.id,{finalizada_cuenta:true},session.access_token);setMisOfertasEnv(prev=>prev.map(x=>x.id===o.id?{...x,finalizada_cuenta:true}:x));}catch(e){alert(e.message);}}} style={{background:"none",border:"1px solid #4ECB7133",borderRadius:8,color:C.success,padding:"4px 10px",cursor:"pointer",fontSize:11,fontFamily:FONT}}>Finalizar</button>
                    </div>
                  )}
                </div>
                );
...
export default function App(){
  ...
  const refreshUnread=useCallback(()=>{
    ...
    Promise.all([
      ...
      sb.getMisOfertas(session.user.email,session.access_token).catch(()=>[])
    ]).then(([msgs,ofertas,nfs,misOfertas])=>{
      ...
      // Contar notifs normales
      const vistasKey="ofertasAceptadasVistas_"+session.user.email;
      let vistas=[];try{vistas=JSON.parse(localStorage.getItem(vistasKey)||"[]");}catch{}
      const nuevasAcept=(misOfertas||[]).filter(o=>o.estado==="aceptada"&&!vistas.includes(o.id)).length;
      setNotifCount(nfs.length);setNotifs(nfs);
      setOfertasAceptadasNuevas(nuevasAcept);
    }).catch(()=>{});
  },[session]);
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"66px 13px 22px":"30px 28px",minHeight:"100vh",maxWidth:`calc(100vw - ${SW}px)`}}>
        <div style={{maxWidth:820,margin:"0 auto"}}>
          ...
          {page==="cuenta"&&<MiCuentaPage key={myPostsKey} session={session} onOpenDetail={setDetailPost} onOpenCurso={setCursoPost} onEdit={p=>{setEditPost(p);setShowForm(true);}} onNew={()=>{setEditPost(null);setShowForm(true);}} onOpenChat={openChat} onRefreshOfertas={refreshUnread} onClearBadge={()=>{setOfertasAceptadasNuevas(0);}}/>}
        </div>
      </main>
