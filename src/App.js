...
class SafeWrapper extends React.Component {
  ...
  render(){
    if(this.state.crashed)return(
      <div style={{background:"#E05C5C15",border:"1px solid #E05C5C33",borderRadius:10,padding:"10px 13px",fontSize:12,color:"#E05C5C"}}>
        ⚠ Este componente no pudo cargar. {this.state.msg?.includes("does not exist")||this.state.msg?.includes("relation")?"Falta crear la tabla quiz_entregas en la DB.":""}
      </div>
    );
...
function CursoPage({post,session,onClose,onUpdatePost}){
  ...
  const [showAdd,setShowAdd]=useState(false);const [showQuizCreator,setShowQuizCreator]=useState(false);const [nuevoTipo,setNuevoTipo]=useState("video");const [nuevoTitulo,setNuevoTitulo]=useState("");const [nuevoUrl,setNuevoUrl]=useState("");const [nuevoTexto,setNuevoTexto]=useState("");const [savingC,setSavingC]=useState(false);
        ...
        <div>
          ...
          <div id="contenido" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 18px",marginBottom:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              ...
              {(esMio||esAyudante)&&<div style={{display:"flex",gap:6}}><button onClick={()=>setShowAdd(v=>!v)} style={{background:C.accentDim,border:"1px solid #F5C84233",borderRadius:8,color:C.accent,padding:"5px 12px",cursor:"pointer",fontSize:12,fontFamily:FONT,fontWeight:600}}>+ Agregar</button><button onClick={()=>setShowQuizCreator(v=>!v)} style={{background:"#C85CE015",border:"1px solid #C85CE033",borderRadius:8,color:C.purple,padding:"5px 12px",cursor:"pointer",fontSize:12,fontFamily:FONT,fontWeight:600}}>📝 Quiz</button></div>}
            </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {contenido.map((c,i)=>{
                  ...
                  if(c.tipo==="quiz"){
                    ...
                    return(<div key={c.id} style={{background:C.card,border:`1px solid ${C.purple}33`,borderRadius:12,padding:"12px 15px"}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:4}}>
                        ...
                        {(esMio||esAyudante)&&<button onClick={()=>removeContenido(c.id)} style={{background:"none",border:"none",color:C.danger,fontSize:15,cursor:"pointer"}}>×</button>}
                      </div>
                      ...
                      {!tieneAcceso&&<div style={{fontSize:11,color:C.muted}}>Inscribite para acceder al quiz.</div>}
                    </div>);
                  }
          ...
          {(esMio||esAyudante)&&contenido.some(c=>c.tipo==="quiz")&&inscripciones.length>0&&(
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      const data={tipo,materia,titulo,descripcion,autor_id:session.user.id,activo:true,verificado,modo:modoDb,modalidad:modalidadForm||null,nivel:nivel||null};
