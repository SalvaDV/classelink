...
function Sidebar({page,setPage,session,onLogout,onNewPost,unreadCount,ofertasCount,mobile,open,onClose}){
  ...
  const nav=[
    ...
    {id:"myposts",icon:"◧",label:"Mis publicaciones",badge:ofertasCount},
...
function ReseñasSeccion({post,session,inscripcion,esMio}){
  ...
  const puedeResena=!esMio&&(inscripcion?.clase_finalizada||inscripcion);
  const enviar=async()=>{
...
// ─── MI CUENTA PAGE ────────────────────────────────────────────────────────────
function MiCuentaPage({session,onOpenDetail}){
  const [pubs,setPubs]=useState([]);const [reseñas,setReseñas]=useState([]);const [docs,setDocs]=useState([]);const [loading,setLoading]=useState(true);
  ...
  const cargar=useCallback(async()=>{
    ...
    try{
      const [p,r,d]=await Promise.all([
        sb.getPublicaciones({autor:email},session.access_token),
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
        ...
        {showAddDoc&&(
          <div style={{background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:14}}>
            ...
            <textarea value={docDesc} onChange={e=>setDocDesc(e.target.value)} placeholder="Descripción adicional (opcional)" style={{...iS,minHeight:55,resize:"vertical"}}/>
      ...
      {/* Mis publicaciones */}
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
        <h3 style={{color:C.text,fontSize:16,fontWeight:700,margin:"0 0 14px"}}>Mis publicaciones ({pubs.length})</h3>
        {loading?<Spinner/>:pubs.length===0?<div style={{color:C.muted,fontSize:13,textAlign:"center",padding:"20px 0"}}>Todavía no publicaste nada.</div>:(
          <div style={{display:"flex",flexDirection:"column",gap:9}}>
            {pubs.map(p=>(<div key={p.id} onClick={()=>onOpenDetail(p)} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"11px 14px",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor=C.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
              <div style={{display:"flex",gap:7,alignItems:"center",marginBottom:4,flexWrap:"wrap"}}><Tag tipo={p.tipo}/>{p.verificado&&<VerifiedBadge/>}<StatusBadge activo={p.activo!==false} finalizado={!!p.finalizado}/><span style={{marginLeft:"auto",fontSize:11,color:C.muted}}>{fmt(p.created_at)}</span></div>
              <div style={{fontWeight:700,color:C.text,fontSize:13,marginBottom:2}}>{p.titulo}</div>
              <div style={{fontSize:12,color:C.muted}}>{p.materia}{p.precio?` · ${fmtPrice(p.precio)}`:""}</div>
            </div>))}
          </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"66px 13px 22px":"30px 28px",minHeight:"100vh",maxWidth:`calc(100vw - ${SW}px)`}}>
        <div style={{maxWidth:820,margin:"0 auto"}}>
          ...
          {page==="cuenta"&&<MiCuentaPage session={session} onOpenDetail={setDetailPost}/>}
        </div>
      </main>
