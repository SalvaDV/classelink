...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil}){
  ...
  const [filtroDurMin,setFiltroDurMin]=useState(0);
  const [maxDurSemanas,setMaxDurSemanas]=useState(0);
  ...
  const filtered=posts.filter(p=>{
    ...
    if(filtroDurMin>0&&p.fecha_inicio&&p.fecha_fin){
      const sem=Math.ceil((new Date(p.fecha_fin)-new Date(p.fecha_inicio))/604800000);
      if(sem<filtroDurMin)return false;
    }
    return true;
  });
  ...
  const activeFilters=[
    ...
    filtroDurMin>0&&`≥${filtroDurMin} sem.`,
  ].filter(Boolean);
  ...
  const FC=({label,active,onClick})=>(
    <button onClick={onClick} style={{padding:"5px 11px",borderRadius:20,fontSize:11,fontWeight:active?700:400,cursor:"pointer",fontFamily:FONT,background:active?C.accent:C.surface,color:active?"#0D0D0D":C.muted,border:`1px solid ${active?C.accent:C.border}`,marginBottom:5,marginRight:5,transition:"all .12s"}}>
      {label}
    </button>
  );
      ...
      <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
        {panelOpen&&(
          <div style={{width:210,flexShrink:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 14px",position:"sticky",top:16,maxHeight:"calc(100vh - 80px)",overflowY:"auto"}}>
            ...
            <div style={{marginBottom:14}}><FL ch="Tipo"/>
              <div style={{display:"flex",flexWrap:"wrap"}}>
                {[["all","Todo"],["oferta","Ofertas"],["busqueda","Búsquedas"]].map(([v,l])=>(
                  <FC key={v} label={l} active={filtroTipo===v} onClick={()=>{setFiltroTipo(v);if(v!=="oferta"){setFiltroModo("all");setFiltroSinc("all");}}}/>
                ))}
              </div>
            </div>
            {(filtroTipo==="all"||filtroTipo==="oferta")&&(
              <div style={{marginBottom:14}}><FL ch="Modalidad"/>
                <div style={{display:"flex",flexWrap:"wrap"}}>
                  {[["all","Todas"],["curso","Cursos"],["particular","Clases part."]].map(([v,l])=>(
                    <FC key={v} label={l} active={filtroModo===v} onClick={()=>{setFiltroModo(v);if(v!=="curso")setFiltroSinc("all");}}/>
                  ))}
                </div>
              </div>
            )}
            {(filtroModo==="curso"||filtroModo==="all")&&(filtroTipo==="all"||filtroTipo==="oferta")&&(
              <div style={{marginBottom:14}}><FL ch="Sincronismo"/>
                <div style={{display:"flex",flexWrap:"wrap"}}>
                  {[["all","Todos"],["sinc","Sincrónico"],["asinc","Asincrónico"]].map(([v,l])=>(
                    <FC key={v} label={l} active={filtroSinc===v} onClick={()=>setFiltroSinc(v)}/>
                  ))}
                </div>
              </div>
            )}
            ...
            {precioMax>0&&(
              <div style={{marginBottom:14}}><FL ch="Precio por hora"/>
            ...
            {maxDurSemanas>0&&(
              <div style={{marginBottom:14}}><FL ch="Duración mínima"/>
                <select value={filtroDurMin} onChange={e=>setFiltroDurMin(+e.target.value)} style={selS}>
                  <option value={0}>Cualquier duración</option>
                  {[1,2,4,8,12,16].filter(v=>v<maxDurSemanas).map(v=><option key={v} value={v}>{v} semana{v!==1?"s":""}</option>)}
                </select>
              </div>
            )}
          </div>
            ...
            {clasesAcordadas.map(o=>{
              ...
              return(
                <div key={o.id} onClick={()=>setEspacioActivo(o)}
                  ...
                  <div style={{width:44,height:44,borderRadius:11,background:"#4ECB7115",border:"1px solid #4ECB7133",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,color:C.success,fontWeight:700,flexShrink:0}}>
                    {soyDoc?"✦":"◈"}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
...
function AyudanteBuscador({post,session,ayudantesActuales,onUpdate}){
  // ayudantesActuales es array de EMAILS (no UUIDs)
  ...
  const agregar=async()=>{
    ...
    if(ayudantesActuales.includes(email)){setErr("Ya es co-docente de este curso");return;}
    if(email===session.user.email){setErr("No podés agregarte como co-docente de tu propio curso");return;}
    setSaving(true);setErr("");
    try{
      const newAyuds=[...ayudantesActuales,email];
      await sb.updatePublicacion(post.id,{ayudantes:newAyuds},session.access_token);
      // Notificar
      sb.insertNotificacion({usuario_id:null,alumno_email:email,tipo:"nuevo_ayudante",publicacion_id:post.id,pub_titulo:post.titulo,leida:false},session.access_token).catch(()=>{});
      onUpdate(newAyuds);setEmailInput("");setErr("");
    }catch(e){setErr("Error: "+e.message);}finally{setSaving(false);}
  };
  const quitar=async(email)=>{
    const newAyuds=ayudantesActuales.filter(a=>a!==email);
    ...
    <div style={{marginTop:14,borderTop:`1px solid ${C.border}`,paddingTop:14}}>
      ...
      {ayudantesActuales.length>0&&(
        <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:10}}>
          {ayudantesActuales.map(email=>(
            <div key={email} style={{display:"flex",alignItems:"center",gap:8,background:"#C85CE015",border:"1px solid #C85CE033",borderRadius:10,padding:"6px 11px"}}>
              <Avatar letra={email[0]} size={22}/>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:12,color:C.text,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{sb.getDisplayName(email)}</div>
                <div style={{fontSize:10,color:C.muted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{email}</div>
              </div>
              <button onClick={()=>quitar(email)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:7,color:C.muted,fontSize:11,cursor:"pointer",padding:"2px 8px",fontFamily:FONT,flexShrink:0}}>Quitar</button>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{fontSize:10,color:C.muted,marginTop:6}}>Los co-docentes pueden subir contenido y ver inscriptos sin pagar ni inscribirse.</div>
    </div>
  );
}
...
function OnboardingModal({session,onClose}){
  ...
  const steps=[
    {title:"¡Bienvenido a ClasseLink!",sub:"La plataforma para conectar estudiantes y docentes.",body:(
      <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:8}}>
        <p style={{color:C.muted,fontSize:13,lineHeight:1.7}}>En ClasseLink podés publicar cursos o clases particulares, buscar docentes para lo que querés aprender, y acordar clases directamente en la plataforma.</p>
        <p style={{color:C.muted,fontSize:13}}>¿Cuál es tu rol principal?</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:4}}>
          {[["docente","Soy docente","Quiero publicar cursos y clases","★"],["alumno","Soy estudiante","Quiero encontrar clases y docentes","◈"]].map(([v,t,s,ic])=>(
          ...
          <button onClick={()=>setRol("ambos")} style={{gridColumn:"1/-1",background:rol==="ambos"?C.accentDim:C.surface,border:`2px solid ${rol==="ambos"?C.accent:C.border}`,borderRadius:14,padding:"10px 12px",cursor:"pointer",fontFamily:FONT,transition:"all .15s"}}>
    ...
    {title:"¿Qué materias te interesan?",sub:"Vamos a mostrar primero lo más relevante para vos.",body:(
      <div style={{display:"flex",flexWrap:"wrap",gap:7,marginTop:12}}>
        {MATERIAS.map(m=>(
          <button key={m} onClick={()=>toggleM(m)} style={{padding:"6px 13px",borderRadius:20,fontSize:12,cursor:"pointer",fontFamily:FONT,background:materias.includes(m)?C.accent:C.surface,color:materias.includes(m)?"#0D0D0D":C.muted,border:`1px solid ${materias.includes(m)?C.accent:C.border}`,fontWeight:materias.includes(m)?700:400,transition:"all .15s"}}>
            {m}
          </button>
        ))}
      </div>
    ...
    {title:"¡Todo listo!",sub:"Ya podés empezar a explorar ClasseLink.",body:(
      <div style={{textAlign:"center",padding:"16px 0"}}>
        ...
        <p style={{color:C.muted,fontSize:13,lineHeight:1.8}}>
          {rol==="docente"?"Usá el botón "+ Publicar" para crear tu primera clase o curso.":rol==="alumno"?"Explorá las publicaciones y contactá a los docentes que te interesen.":"Podés explorar y publicar clases según lo que necesites."}<br/>
          <span style={{color:C.muted,fontSize:12}}>Tus preferencias se guardaron.</span>
        </p>
      </div>
    )},
  ];
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#000d",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FONT,padding:20}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:22,width:"min(480px,96vw)",overflow:"hidden"}}>
        {/* Progress bar */}
        ...
        <div style={{padding:"24px 26px 26px"}}>
          ...
          <div style={{textAlign:"center",marginTop:10}}><button onClick={finish} style={{background:"none",border:"none",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:FONT}}>Saltar por ahora</button></div>
        </div>
      </div>
    </div>
  );
...
// ─── DOCENTE STATS — panel de estadísticas para docentes ─────────────────────
...
// ─── ESPACIO CLASE MODAL — pantalla privada para oferta aceptada ──────────────
// FK FIX: el chat usa publicacion_id = la busqueda_id (que sí está en publicaciones)
// porque las búsquedas también son publicaciones con tipo="busqueda"
function EspacioClaseModal({oferta,session,onClose}){
  ...
  const otroNombre=soyDocente
    ?(oferta.busqueda_autor_nombre||otroEmail?.split("@")[0])
    :(oferta.ofertante_nombre||otroEmail?.split("@")[0]);
  ...
  const pageRef=useRef(null);
  // Usamos oferta.id para contenido (no tiene FK constraint de publicaciones)
  // y oferta.busqueda_id para mensajes (sí está en publicaciones)
    ...
    <div ref={pageRef} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      <div style={{position:"sticky",top:0,zIndex:10,background:C.sidebar,borderBottom:`1px solid ${C.border}`,padding:"12px 20px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11,color:C.muted}}>{soyDocente?"Alumno":"Docente"}: {otroNombre}</div>
        </div>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"24px 20px"}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px 20px",marginBottom:20}}>
          ...
          {soyDocente&&showAdd&&(
            <div style={{background:"#111",border:`1px solid ${C.border}`,borderRadius:12,padding:"14px",marginBottom:14}}>
              ...
              <textarea value={nuevoBody} onChange={e=>setNuevoBody(e.target.value)} placeholder={nuevoTipo==="link"||nuevoTipo==="video"?"URL del enlace":"Texto o descripción (opcional)"} style={{...iS2,minHeight:65,resize:"vertical"}}/>
        ...
        {/* Chat — usa busqueda_id que sí es FK válido en publicaciones */}
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden"}}>
...
// ─── ESPACIO CHAT ─────────────────────────────────────────────────────────────
// usa busqueda_id (publicacion válida) como canal
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>(
                <div key={o.id} style={{background:C.surface,border:`1px solid ${o.estado==="rechazada"?"#E05C5C33":o.estado==="aceptada"?"#4ECB7133":C.border}`,borderRadius:12,padding:"11px 14px",display:"flex",gap:10,alignItems:"center"}}>
                  ...
                  {o.estado==="aceptada"&&(
                    <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0}}>
                      <button onClick={()=>setEspacioModal({...o,_rol:"docente"})} style={{background:C.accent,border:"none",borderRadius:8,color:"#0D0D0D",padding:"6px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>Ver espacio →</button>
