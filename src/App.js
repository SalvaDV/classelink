...
function InscripcionesPage({session,onOpenCurso,onOpenChat,onMarkNotifsRead}){
  ...
  const marcarNotifPubLeida=async(pubId)=>{
    ...
    try{
      await sb.marcarNotifsTipoLeidas(session.user.email,["valorar_curso","nuevo_ayudante"],session.access_token);
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {(()=>{
        ...
        // Badge de texto (novedad) se trackea en localStorage; borde se mantiene por estado
        ...
        const marcarVista=(o)=>{
          const k=vistaId(o);
          if(!vistas.includes(k)){
            const nv=[...vistas,k];
            localStorage.setItem(vistasKey,JSON.stringify(nv));
            vistas=nv;// actualizar local para re-renders dentro del mismo bloque
          }
        ...
        return(
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>{
                // Badge de novedad: solo aceptadas/rechazadas/contraoferta-alumno no vistas
                const esNovedad=!vistas.includes(vistaId(o))&&(
                ...
                // Borde: siempre por estado (no desaparece)
                const borderColor=o.estado==="aceptada"?"#4ECB7133":o.estado==="rechazada"?"#E05C5C33":o.estado==="pendiente"&&o.contraoferta_precio&&o.contraoferta_de==="alumno"?"#C85CE033":C.border;
                return(
                <div key={o.id} onClick={()=>{if(esNovedad)marcarVista(o);}} style={{background:C.surface,border:`1px solid ${borderColor}`,borderRadius:12,padding:"11px 14px",display:"flex",gap:10,alignItems:"center",position:"relative",overflow:"hidden",cursor:esNovedad?"pointer":"default"}}>
                  ...
                  <div style={{flex:1,minWidth:0,paddingLeft:6}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:2}}>
                      ...
                      {esNovedad&&<span style={{fontSize:10,fontWeight:700,background:o.estado==="aceptada"?"#4ECB7122":o.estado==="rechazada"?"#E05C5C22":"#C85CE022",color:o.estado==="aceptada"?C.success:o.estado==="rechazada"?C.danger:"#C85CE0",border:`1px solid ${o.estado==="aceptada"?"#4ECB7155":o.estado==="rechazada"?"#E05C5C55":"#C85CE055"}`,borderRadius:20,padding:"2px 8px",flexShrink:0}}>
                        {o.estado==="aceptada"?"✓ Aceptada":o.estado==="rechazada"?"✗ Rechazada":"↔ Contraoferta"}
                      </span>}
                    </div>
                  ...
                  {o.estado==="pendiente"&&o.contraoferta_precio&&o.contraoferta_de==="alumno"&&(
                    <span onClick={e=>e.stopPropagation()}><ContraRespondedor oferta={o} session={session} onActualizado={()=>{marcarVista(o);cargar();}}/></span>
                  )}
...
export default function App(){
  ...
  const refreshUnread=useCallback(()=>{
    ...
    Promise.all([
      ...
      // Notifs para Mis inscripciones (valorar_curso, nuevo_ayudante)
      const notifsInsc=(nfs||[]).filter(n=>n.tipo==="valorar_curso"||n.tipo==="nuevo_ayudante");
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"66px 13px 22px":"30px 28px",minHeight:"100vh",maxWidth:`calc(100vw - ${SW}px)`}}>
        <div style={{maxWidth:820,margin:"0 auto"}}>
          ...
          {page==="inscripciones"&&<InscripcionesPage session={session} onOpenCurso={setCursoPost} onOpenChat={openChat} onMarkNotifsRead={()=>{sb.marcarNotifsTipoLeidas(session.user.email,["valorar_curso","nuevo_ayudante"],session.access_token).then(refreshUnread).catch(()=>{});}}/>}
          {page==="cuenta"&&<MiCuentaPage key={myPostsKey} session={session} onOpenDetail={setDetailPost} onOpenCurso={setCursoPost} onEdit={p=>{setEditPost(p);setShowForm(true);}} onNew={()=>{setEditPost(null);setShowForm(true);}} onOpenChat={openChat} onRefreshOfertas={refreshUnread} onClearBadge={()=>{
            ...
            // Marcar notifs de cuenta como leídas en server
