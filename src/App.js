...
// Cache de avatares por email — se llena cuando se carga el perfil del autor
const _avatarCache={};
const useAutorAvatar=(email,token)=>{
  ...
  useEffect(()=>{
    if(!email)return;
    // Primero usar localStorage (instantáneo)
  ...
  const inner=(
    <div style={{width:224,height:"100%",background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`,background:`linear-gradient(135deg,${C.accentDim},${C.bg})`}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          {session.avatarUrl&&session.avatarUrl.startsWith("http")
            ?<div style={{width:40,height:40,borderRadius:"50%",overflow:"hidden",flexShrink:0,border:`2px solid ${C.accent}40`}}><img src={session.avatarUrl} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}} onError={e=>e.target.style.display="none"}/></div>
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      // ── Disparar alertas con matching semántico IA (nueva pub Y edición) ──
      if(savedPub){
        (async()=>{
          try{
            const alertas=await sb.db(
            ...
            // Construir perfil completo de la publicación nueva
            const pubPerfil={
              titulo:savedPub.titulo||"",
              descripcion:(savedPub.descripcion||"").slice(0,300),
              materia:savedPub.materia||"",
              tipo:savedPub.tipo||"oferta",
              modalidad:savedPub.modalidad||"", // "virtual" | "presencial" | "mixto"
              ubicacion:savedPub.ubicacion||"",
              precio:savedPub.precio?fmtPrice(savedPub.precio,savedPub.moneda):"Gratis",
              nivel:savedPub.nivel||"",
              certificado:savedPub.otorga_certificado?"Sí":"No",
              frecuencia:savedPub.frecuencia||"",
            };

            for(const alerta of alertas){
              try{
                // Filtro rápido por tipo — sin gastar tokens de IA
                const tipoAlerta=alerta.tipo_alerta||"ambos";
                if(tipoAlerta!=="ambos"&&pubPerfil.tipo!==tipoAlerta)continue;

                // Extraer criterios estructurados de la alerta para darle contexto a la IA
                let criterios={};
                try{criterios=JSON.parse(alerta.criterios_json||"{}");}catch{}

                // Construir contexto de la alerta con TODOS sus atributos
                const alertaContexto=[
                  `Descripción libre del usuario: "${alerta.descripcion}"`,
                  ...
                  criterios.palabras_clave?.length?`Palabras clave detectadas: ${criterios.palabras_clave.join(", ")}`:"",
                ...
                const pubContexto=[
                  ...
                  pubPerfil.ubicacion?`Ubicación: ${pubPerfil.ubicacion}`:"Sin ubicación especificada",
                ...
                const raw=await sb.callIA(
                  `Sos un sistema de matching inteligente para alertas educativas en Argentina.
Tu tarea: evaluar si una publicación nueva es relevante para lo que un usuario está buscando.

REGLAS DE EVALUACIÓN:
- Temática: considerá sinónimos, conceptos relacionados, temas que se solapan. "Historia de la 2da guerra mundial" es relevante para alguien que busca "guerras mundiales" o "historia del siglo XX".
- Modalidad: si el usuario pidió "presencial", una clase virtual NO es relevante (a menos que diga "o virtual"). Si no especificó, aceptar cualquiera.
- Ubicación: si el usuario mencionó una ciudad o "cerca de X", verificá que la publicación sea de esa zona O sea virtual. Si la publicación no tiene ubicación especificada y es presencial, es una coincidencia débil.
- Precio: si el usuario mencionó un rango de precio, considerarlo. Si no, ignorarlo.
- Certificado: si el usuario explícitamente quiere certificado, verificar que la publicación lo otorgue.
- Nivel: si el usuario mencionó nivel (principiante, avanzado, etc.), verificar compatibilidad.
- NO es necesario que coincida todo — si el TEMA coincide bien y no hay contradicciones mayores, es relevante.
- Una coincidencia temática sola ya puede ser suficiente si no hay restricciones incompatibles.

Respondé SOLO con JSON: {"match":true,"razon":"frase corta"} o {"match":false,"razon":"por qué no"}`,
                  `ALERTA DEL USUARIO:\n${alertaContexto}\n\nPUBLICACIÓN NUEVA:\n${pubContexto}`,
                ...
                try{
                  ...
                  match=raw.includes('"match":true')||raw.includes('"match": true');
                }
                console.log(`[Alerta] "${alerta.descripcion}" → pub:"${savedPub.titulo}" → match:${match} razon:${razon}`);

                if(match){
                  sb.sendEmail("alerta_publicacion",alerta.usuario_email,{
                    pub_titulo:savedPub.titulo,
                    materia:savedPub.materia||"",
                    tipo:savedPub.tipo==="oferta"?"Clase/Curso":"Búsqueda",
                    precio:pubPerfil.precio,
                    modalidad:savedPub.modalidad||"",
                    descripcion:(savedPub.descripcion||"").slice(0,150),
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={async()=>{
                  ...
                  try{
                    ...
                    _avatarCache[session.user.email]=avatarUrl.trim()||null;
                    // Guardar en localStorage para persistir entre recargas
                    try{if(avatarUrl.trim())localStorage.setItem("cl_avatar_"+session.user.email,avatarUrl.trim());else localStorage.removeItem("cl_avatar_"+session.user.email);}catch{}
...
export default function App(){
  ...
  useEffect(()=>{
    if(!session)return;
    try{
      const done=localStorage.getItem("cl_onboarding_done_"+session.user.email);
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"62px 12px 70px":"24px 24px 24px",minHeight:"100vh",width:`calc(100vw - ${SW}px)`,maxWidth:`calc(100vw - ${SW}px)`,boxSizing:"border-box",background:C.bg}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          {page==="cuenta"&&<MiCuentaPage key={myPostsKey} session={session} onOpenDetail={setDetailPost} onOpenCurso={setCursoPost} onEdit={p=>{setEditPost(p);setShowForm(true);}} onNew={()=>{setEditPost(null);setShowForm(true);}} onOpenChat={openChat} onRefreshOfertas={refreshUnread} onStartOnboarding={()=>setShowOnboarding(true)} onClearBadge={()=>{
