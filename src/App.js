...
function MiCuentaPage({session,onOpenDetail,onOpenCurso,onEdit,onNew,onOpenChat,onRefreshOfertas}){
  ...
  const cargar=useCallback(async()=>{
    setLoading(true);
    try{
      const [p,r,d,ofertasRaw,misOEnv]=await Promise.all([
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {(()=>{
        const visibles=misOfertasEnv.filter(o=>!descartadas.includes(o.id)&&(o.estado!=="rechazada"||(o.estado==="rechazada"&&!descartadas.includes(o.id))));
        // mostrar: pendientes siempre, aceptadas hasta que haya inscripcion (no lo sabemos aquí, mostrar siempre), rechazadas hasta que se descarte
        const mostrar=visibles.filter(o=>o.estado==="pendiente"||o.estado==="aceptada"||o.estado==="rechazada");
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>(
                <div key={o.id} style={{background:C.surface,border:`1px solid ${o.estado==="rechazada"?"#E05C5C33":o.estado==="aceptada"?"#4ECB7133":C.border}`,borderRadius:12,padding:"11px 14px",display:"flex",gap:10,alignItems:"center"}}>
                  ...
                  {o.estado==="aceptada"&&<span style={{fontSize:11,color:C.success,fontWeight:600,flexShrink:0}}>💬 Podés chatear</span>}
                </div>
              ))}
            </div>
          </div>
