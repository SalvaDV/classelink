...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil,onOpenCurso}){
  ...
  const filtered=posts.filter(p=>{
    ...
    if(busquedaDebounced&&!p.titulo?.toLowerCase().includes(q)&&!p.descripcion?.toLowerCase().includes(q)&&!(p.autor_nombre||safeDisplayName(p.autor_nombre,p.autor_email)||"").toLowerCase().includes(q))return false;
...
<button onClick={()=>setMostrarRechazadas(false)} style={{background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",lineHeight:1,padding:"0 4px",flexShrink:0}} title="Ocultar">×</button>
            </div>
          )}
  ...
  const handleClickEliminar=async()=>{
    if(post.tipo==="busqueda"){
      try{
        ...
        setOfertaAceptadaInfo(aceptada?{nombre:aceptada.ofertante_nombre||safeDisplayName(aceptada.ofertante_nombre,aceptada.ofertante_email),email:aceptada.ofertante_email}:null);
...
function DetailModal({post,session,onClose,onChat,onOpenCurso,onOpenPerfil}){
  ...
  const nombre=post.autor_nombre||sb.getDisplayName(post.autor_email)||safeDisplayName(post.autor_nombre,post.autor_email)||"Usuario";const esMio=post.autor_email===session.user.email;
...
function MiCuentaPage({session,onOpenDetail,onOpenCurso,onEdit,onNew,onOpenChat,onRefreshOfertas,onClearBadge}){
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {/* ── TABS DE NAVEGACIÓN ── */}
      <div style={{display:"flex",gap:0,marginBottom:16,borderBottom:`2px solid ${C.border}`,background:C.surface,borderRadius:"10px 10px 0 0",padding:"0 4px"}}>
        {[
