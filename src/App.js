...
function InscribirseBtn({post,session,onDone}){
  ...
  return(
    <button onClick={inscribirse} disabled={loading}
      style={{background:tienePrecio?"linear-gradient(135deg,#009EE3,#0070BA)":C.success+"22",color:tienePrecio?"#fff":C.success,border:tienePrecio?"none":`1px solid ${C.success}44`,borderRadius:11,padding:"8px 16px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT,display:"flex",alignItems:"center",gap:6,boxShadow:tienePrecio?"0 4px 12px rgba(0,158,227,.3)":"none"}}>
      {loading?"...":(tienePrecio?<>💳 Inscribirme · {fmtPrice(post.precio,post.moneda)}</>:"Inscribirme")}
    </button>
  );
}
          ...
          <div style={{flex:"0 0 300px",minWidth:260,position:"sticky",top:72}}>
            <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"24px",boxShadow:"0 4px 24px rgba(0,0,0,.08)"}}>
              ...
              <div style={{marginTop:16,paddingTop:14,borderTop:`1px solid ${C.border}`,display:"flex",flexDirection:"column",gap:8}}>
                {/* ── Pagar con Mercado Pago ── */}
                {post.precio&&!esMio&&!inscripcion&&!post.finalizado&&post.tipo==="oferta"&&(
