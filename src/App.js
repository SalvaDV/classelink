...
function Sidebar({page,setPage,session,onLogout,onNewPost,unreadCount,ofertasCount,mobile,open,onClose}){
  ...
  const nav=[
    ...
    {id:"inscripciones",icon:"📚",label:"Mis inscripciones"},
...
function OfertarBtn({post,session}){
  ...
  const enviar=async()=>{
    ...
    try{
      await sb.insertOfertaBusq({busqueda_id:post.id,busqueda_autor_email:post.autor_email,ofertante_email:session.user.email,ofertante_nombre:session.user.email.split("@")[0],precio:precio?parseFloat(precio):null,precio_tipo:tipo,mensaje:msg,leida:false,estado:"pendiente"},session.access_token);
...
function PostChatBtn({post,session,onOpenChat}){
  ...
  if(!permitido)return <span style={{fontSize:11,color:C.muted,fontStyle:"italic"}}>Inscribite para chatear</span>;
    ...
    <div style={{background:C.card,border:`1px solid ${ofertasPendientes>0?C.accent:C.border}`,borderRadius:14,padding:"14px 16px",position:"relative",overflow:"hidden",fontFamily:FONT}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:10}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:4,flexShrink:0,minWidth:95}}>
          ...
          {!finalizado&&<button onClick={()=>onEdit(post)} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,padding:"5px 8px",cursor:"pointer",fontSize:11,fontWeight:600,fontFamily:FONT}}>✏️ Editar</button>}
...
function FinalizarClaseModal({post,session,onClose,onFinalizado}){
  ...
  const finalizar=async()=>{setSaving(true);try{await Promise.all(inscripciones.map(ins=>sb.updateInscripcion(ins.id,{clase_finalizada:true,fecha_finalizacion:new Date().toISOString()},session.access_token)));await Promise.all(inscripciones.map(ins=>sb.insertNotificacion({tipo:"valorar_curso",alumno_email:ins.alumno_email,publicacion_id:post.id,pub_titulo:post.titulo,leida:false}).catch(()=>{})));onFinalizado();onClose();}finally{setSaving(false);}};
  return(
...
function ReseñasSeccion({post,session,inscripcion,esMio}){
  ...
  const puedeResena=!esMio&&inscripcion?.clase_finalizada;
  ...
  const enviar=async()=>{
    ...
    try{await sb.insertReseña({publicacion_id:post.id,autor_nombre:session.user.email.split("@")[0],autor_pub_email:post.autor_email,texto:reseña,estrellas:estrella},session.access_token);
...
function ChatCurso({post,session,inscripcion,esMio,ayudantes=[]}){
  const [msgs,setMsgs]=useState([]);const [input,setInput]=useState("");const [loading,setLoading]=useState(true);
  const miEmail=session.user.email;const nombre=miEmail.split("@")[0];
  const bottomRef=useRef(null);
  const cargar=useCallback(async()=>{
    try{
      const data=await sb.getMisChats(miEmail,session.access_token).catch(()=>[]);const filtered=data.filter(m=>m.publicacion_id===post.id&&m.para_nombre==="__grupo__");
      setMsgs(filtered);setTimeout(()=>bottomRef.current?.scrollIntoView({behavior:"smooth"}),50);
    }finally{setLoading(false);}
  ...
  const send=async()=>{
    ...
    try{await sb.insertMensaje({publicacion_id:post.id,de_nombre:miEmail,para_nombre:"__grupo__",texto:txt,leido:true,pub_titulo:post.titulo},session.access_token);cargar();}
    catch(e){alert("Error: "+e.message);}
  };
  const esEspecial=(email)=>email===post.autor_email||ayudantes.includes(email);
  return(
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,overflow:"hidden"}}>
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontWeight:700,color:C.text,fontSize:14}}>Chat del curso</span>
        <span style={{fontSize:11,color:C.muted}}>· visible para todos los inscriptos</span>
      </div>
      <div style={{height:320,overflowY:"auto",padding:"12px 14px",display:"flex",flexDirection:"column",gap:7}}>
        {loading?<Spinner/>:msgs.length===0?<div style={{color:C.muted,fontSize:13,textAlign:"center",padding:20}}>Iniciá la conversación del grupo 👋</div>
          :msgs.map((m,i)=>{
            const spec=esEspecial(m.de_nombre);const esMiMsg=m.de_nombre===miEmail;
            const esOwner=m.de_nombre===post.autor_email;
            return(<div key={i} style={{display:"flex",justifyContent:esMiMsg?"flex-end":"flex-start",gap:6,alignItems:"flex-end"}}>
              {!esMiMsg&&<Avatar letra={m.de_nombre[0]} size={22}/>}
              <div style={{maxWidth:"75%"}}>
                {!esMiMsg&&<div style={{fontSize:10,color:spec?C.accent:C.muted,fontWeight:spec?700:400,marginBottom:2}}>
                  {m.de_nombre.split("@")[0]}{esOwner?" (docente)":ayudantes.includes(m.de_nombre)?" (ayudante)":""}
                </div>}
                <div style={{background:esMiMsg?C.accent:spec?"#F5C84222":C.surface,color:esMiMsg?"#0D0D0D":C.text,padding:"8px 12px",borderRadius:esMiMsg?"13px 13px 4px 13px":"13px 13px 13px 4px",fontSize:12,lineHeight:1.5,border:spec&&!esMiMsg?`1px solid #F5C84244`:`1px solid ${esMiMsg?"transparent":C.border}`}}>
                  {m.texto}
                </div>
              </div>
            </div>);
      ...
      <div style={{padding:"10px 13px",borderTop:`1px solid ${C.border}`,display:"flex",gap:7}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Escribí un mensaje al grupo..." style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:"8px 13px",color:C.text,fontSize:12,outline:"none",fontFamily:FONT}}/>
        <button onClick={send} disabled={!input.trim()} style={{background:C.accent,border:"none",borderRadius:"50%",width:34,height:34,cursor:"pointer",fontSize:15,flexShrink:0,opacity:!input.trim()?.5:1}}>↑</button>
      </div>
    </div>
  );
}
...
function CerrarInscModal({post,session,onClose}){
  const [saving,setSaving]=useState(false);const [ok,setOk]=useState(false);
  const cerrar=async()=>{setSaving(true);try{await sb.updatePublicacion(post.id,{inscripcionesCerradas:true},session.access_token);setOk(true);setTimeout(onClose,1200);}finally{setSaving(false);}};
...
function CursoPage({post,session,onClose}){
  ...
  const [calExpanded,setCalExpanded]=useState(false);const [showFinalizar,setShowFinalizar]=useState(false);const [showDenuncia,setShowDenuncia]=useState(false);const [showCerrarInsc,setShowCerrarInsc]=useState(false);
  ...
  const tieneAcceso=esMio||!!inscripcion;const duracion=calcDuracion(post.fecha_inicio,post.fecha_fin);const hasCal=post.sinc==="sinc"&&post.clases_sinc;
  // Gate: if not owner and not inscribed, show access wall
  if(!tieneAcceso&&!loading&&!inscLoading)return(<div style={{position:"fixed",inset:0,background:C.bg,zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:FONT,gap:16,padding:24}}><button onClick={onClose} style={{position:"absolute",top:20,left:20,background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,color:C.text,padding:"7px 12px",cursor:"pointer",fontSize:13,fontFamily:FONT}}>← Volver</button><div style={{fontSize:48,marginBottom:8}}>🔒</div><h2 style={{color:C.text,fontSize:20,fontWeight:700,margin:0}}>Contenido restringido</h2><p style={{color:C.muted,fontSize:14,textAlign:"center",maxWidth:360,margin:0}}>Debés estar inscripto para acceder al contenido de esta publicación.</p>{inscLoading?<Spinner/>:<Btn onClick={inscribirse} variant="success" style={{padding:"10px 28px",fontSize:14}}>Inscribirme gratis →</Btn>}<button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:13,cursor:"pointer",fontFamily:FONT}}>Volver atrás</button></div>);
    ...
    <div style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      <div style={{position:"sticky",top:0,zIndex:10,background:C.sidebar,borderBottom:`1px solid ${C.border}`,padding:"12px 20px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          {esMio&&!post.finalizado&&<button onClick={()=>setShowCerrarInsc(true)} style={{background:"#E0955C15",border:"1px solid #E0955C33",borderRadius:9,color:C.warn,padding:"6px 12px",cursor:"pointer",fontSize:12,fontFamily:FONT,fontWeight:600}}>Cerrar inscripciones</button>}
          {esMio&&!post.finalizado&&<button onClick={()=>setShowFinalizar(true)} style={{background:"#4ECB7122",border:"1px solid #4ECB7144",borderRadius:9,color:C.success,padding:"6px 12px",cursor:"pointer",fontSize:12,fontFamily:FONT,fontWeight:600}}>Finalizar clase</button>}
          {post.finalizado&&esMio&&<span style={{fontSize:12,color:C.info,fontWeight:600}}>✓ Clase finalizada</span>}
          ...
          {esMio?<span style={{fontSize:12,color:C.muted}}>Sos el docente</span>:(
            inscLoading?<Spinner small/>:(
              post.finalizado?<span style={{fontSize:12,color:C.info}}>Clase finalizada</span>:
              post.inscripcionesCerradas?<span style={{fontSize:12,color:C.muted}}>Inscripciones cerradas</span>:
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px",display:"grid",gridTemplateColumns:hasCal?"1fr 300px":"1fr",gap:22,alignItems:"start"}}>
        <div>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:18}}>
            ...
            <div style={{display:"flex",gap:9,flexWrap:"wrap"}}>{post.precio&&<Chip label="PRECIO" val={fmtPrice(post.precio)}/>}{duracion&&<Chip label="DURACIÓN" val={duracion}/>}{post.fecha_inicio&&<Chip label="INICIO" val={fmt(post.fecha_inicio)}/>}{post.fecha_fin&&<Chip label="FIN" val={fmt(post.fecha_fin)}/>}</div>
          </div>
          ...
          {tieneAcceso&&<div style={{marginBottom:18}}><ChatCurso post={post} session={session} inscripcion={inscripcion} esMio={esMio}/></div>}
        ...
        {hasCal&&(
          <div style={{position:"sticky",top:70}}>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden"}}>
              ...
              <div style={{padding:"14px"}}><CalendarioCurso post={post} compact/></div>
            </div>
          </div>
        )}
      </div>
      {calExpanded&&(<div style={{position:"fixed",inset:0,background:"#000c",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setCalExpanded(false)}><div onClick={e=>e.stopPropagation()} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:"22px",width:"min(500px,95vw)",maxHeight:"90vh",overflowY:"auto"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><h3 style={{color:C.text,margin:0,fontSize:16}}>📅 Calendario</h3><button onClick={()=>setCalExpanded(false)} style={{background:"none",border:"none",color:C.muted,fontSize:21,cursor:"pointer"}}>×</button></div><CalendarioCurso post={post}/></div></div>)}
      {showFinalizar&&<FinalizarClaseModal post={post} session={session} onClose={()=>setShowFinalizar(false)} onFinalizado={()=>{}}/>}
      {showDenuncia&&<DenunciaModal post={post} session={session} onClose={()=>setShowDenuncia(false)}/>}
      {showCerrarInsc&&<CerrarInscModal post={post} session={session} onClose={()=>setShowCerrarInsc(false)}/>}}
    </div>
  );
}
...
function DetailModal({post,session,onClose,onChat,onOpenCurso,onOpenPerfil}){
  ...
  const [reseña,setReseña]=useState("");const [estrella,setEstrella]=useState(5);const [saving,setSaving]=useState(false);const [err,setErr]=useState("");
  const [inscripcion,setInscripcion]=useState(null);const [puedeChat,setPuedeChat]=useState(false);
  ...
  useEffect(()=>{
    Promise.all([
      ...
      post.tipo==="busqueda"?sb.getOfertaAceptada(post.id,session.user.email,session.access_token).catch(()=>[]):Promise.resolve([])
    ]).then(([pub,usr,ins,ofertaAcept])=>{
      setReseñas(pub);setReseñasUsuario(usr);
      const insc=ins.find(i=>i.publicacion_id===post.id)||null;
      setInscripcion(insc);
      setPuedeChat(!!insc||(Array.isArray(ofertaAcept)&&ofertaAcept.length>0));
    }).finally(()=>setLoading(false));
    ...
    <Modal onClose={onClose} width="min(660px,97vw)">
      <div style={{padding:"19px 21px"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:13}}>
          {!esMio&&(puedeChat?<button onClick={()=>{onClose();onChat(post);}} style={{background:C.accent,color:"#0D0D0D",border:"none",borderRadius:11,padding:"8px 16px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT}}>💬 Iniciar conversación</button>:<span style={{fontSize:12,color:C.muted,fontStyle:"italic",alignSelf:"center"}}>{post.tipo==="busqueda"?"Esperando que el autor acepte tu oferta":"Inscribite para poder chatear"}</span>)}
          ...
          {/* OfertarBtn SOLO aquí — en el modal completo, no en la card de lista */}
        ...
        {tipo==="oferta"&&(<>
          ...
          {modo==="curso"&&(<div style={{borderTop:`1px solid ${C.border}`,paddingTop:10}}>
            ...
            <div style={{display:"flex",gap:7,marginBottom:9}}>
              <div style={{flex:1}}><Label>Inicio</Label><input type="date" value={fechaInicio} onChange={e=>setFechaInicio(e.target.value)} style={{...iS,margin:0,colorScheme:"dark"}}/></div>
              <div style={{flex:1}}><Label>Fin</Label><input type="date" value={fechaFin} onChange={e=>setFechaFin(e.target.value)} style={{...iS,margin:0,colorScheme:"dark"}}/></div>
            </div>
...
function PerfilPage({autorEmail,session,onClose,onOpenDetail,onOpenChat}){
  ...
  const savedColor=localStorage.getItem("avatarColor_"+autorEmail);
  ...
  useEffect(()=>{
    ...
    ]).then(([p,r,d])=>{setPubs(p.filter(x=>x.activo!==false));setReseñas(r);setDocs(d||[]);}).finally(()=>setLoading(false));
  },[autorEmail,session]);
    ...
    <div style={{position:"fixed",inset:0,background:C.bg,zIndex:400,overflowY:"auto",fontFamily:FONT}}>
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"24px 20px"}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
          <h3 style={{color:C.text,fontSize:15,fontWeight:700,margin:"0 0 14px"}}>Publicaciones activas</h3>
          {loading?<Spinner/>:pubs.length===0?<div style={{color:C.muted,fontSize:13,textAlign:"center",padding:"24px 0"}}>Sin publicaciones activas.</div>:(
            <div style={{display:"flex",flexDirection:"column",gap:9}}>
              {pubs.map(p=>(<div key={p.id} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px 15px",cursor:"pointer",transition:"border-color .15s"}} onClick={()=>{onClose();setTimeout(()=>onOpenDetail(p),50);}} onMouseEnter={e=>e.currentTarget.style.borderColor=C.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
                ...
                <div style={{fontSize:12,color:C.muted,marginBottom:p.precio?4:0}}>{p.materia}</div>
                {p.precio&&<div style={{fontSize:12,color:C.accent,fontWeight:600}}>💰 {fmtPrice(p.precio)}{p.precio_tipo&&p.modo!=="curso"?` /${p.precio_tipo}`:""}</div>}
              </div>))}
            </div>
          )}
        </div>
...
function MiCuentaPage({session,onOpenDetail,onOpenCurso,onEdit,onNew,onOpenChat,onRefreshOfertas}){
  ...
  // Perfil edición
  const [editingPerfil,setEditingPerfil]=useState(false);const [displayName,setDisplayName]=useState("");const [bio,setBio]=useState("");const [savingPerfil,setSavingPerfil]=useState(false);
  const [avatarColor2,setAvatarColor2]=useState("");
  const email=session.user.email;const nombre=email.split("@")[0];
  ...
  const cargar=useCallback(async()=>{
    setLoading(true);
    try{
      const [p,r,d,ofertasRaw]=await Promise.all([
      ...
      setPubs(p||[]);setReseñas(r||[]);setDocs(d||[]);
  ...
  const addDoc=async()=>{
    ...
    try{
      await sb.insertDocumento({usuario_email:email,tipo:docTipo,titulo:docTitulo,institucion:docInst,año:docAño,descripcion:docDesc},session.access_token);
    ...
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:18,padding:"22px 24px",marginBottom:20}}>
        <div style={{display:"flex",gap:16,alignItems:"flex-start",marginBottom:16}}>
          ...
          <div style={{flex:1}}>
            <h2 style={{color:C.text,fontSize:20,fontWeight:700,margin:"0 0 3px"}}>{nombre}</h2>
          ...
          <button onClick={()=>setEditingPerfil(v=>!v)} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,color:C.muted,padding:"6px 12px",cursor:"pointer",fontSize:12,fontFamily:FONT}}>✏️ Editar</button>
        </div>
        {editingPerfil&&(
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
        ...
        {loading?<Spinner/>:ofertas.length===0?(
          ...
          <>
            {ofertas.filter(p=>p.modo==="particular").length>0&&(
              <div style={{marginBottom:16}}>
                ...
                <div style={{display:"grid",gap:10}}>
                  {ofertas.filter(p=>p.modo==="particular").map(p=>(<div key={p.id}>
                    <MyPostCard post={p} session={session} onEdit={onEdit} onToggle={toggle} onDelete={remove} onOpenCurso={onOpenCurso} toggling={toggling} ofertasPendientes={0}/>
                  </div>))}
                </div>
              </div>
            )}
            {ofertas.filter(p=>p.modo==="curso").length>0&&(
              <div>
                <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>Cursos</div>
                <div style={{display:"grid",gap:10}}>
                  {ofertas.filter(p=>p.modo==="curso").map(p=>(<div key={p.id}>
                    <MyPostCard post={p} session={session} onEdit={onEdit} onToggle={toggle} onDelete={remove} onOpenCurso={onOpenCurso} toggling={toggling} ofertasPendientes={0}/>
                  </div>))}
                </div>
              </div>
            ...
            {busquedas.map(p=>{
              ...
              return(<div key={p.id} style={{background:C.surface,border:`1px solid ${cnt>0?C.accent:C.border}`,borderRadius:14,padding:"14px 16px",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:4,flexShrink:0,minWidth:95}}>
                    {cnt>0&&<button onClick={()=>setOfertasModal(p)} style={{background:C.accentDim,border:`1px solid #F5C84244`,borderRadius:8,color:C.accent,padding:"5px 8px",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:FONT}}>Ver ofertas</button>}
...
function ChatBotWidget(){
  ...
  const FAQS=[
    {q:"¿Cómo me inscribo a un curso?",a:"En la página del curso encontrás el botón 'Inscribirme gratis'. Hacé clic y listo, tendrás acceso a todo el contenido."},
    {q:"¿Cómo publico una oferta?",a:"Hacé clic en '+ Nueva publicación' en el menú lateral. Elegí 'Ofrezco clases', completá el formulario y publicá."},
    {q:"¿Cómo verifico mi perfil?",a:"Al crear una publicación de oferta, el sistema te hace una pregunta sobre tu materia. Si respondés correctamente, obtenés la insignia ✓ Verificado."},
    {q:"¿Cómo contacto a un docente?",a:"Para contactar a un docente debés estar inscripto en su clase o curso. Una vez inscripto, aparecerá el botón '💬 Contactar'."},
    {q:"¿Cómo finalizo una clase?",a:"Entrá a tu publicación desde Mi cuenta, abrí la página del curso y usá el botón '✅ Finalizar' en la barra superior."},
    {q:"¿Cómo dejo una reseña?",a:"Las reseñas se habilitan cuando el docente finaliza las clases. Entrá al curso y podrás valorar desde la sección de Reseñas."},
    {q:"¿Cómo funciona el sistema de ofertas?",a:"Si publicás una búsqueda, los docentes pueden enviarte una oferta. Vas a ver una notificación en Mi cuenta y podés aceptarla o rechazarla."},
    {q:"¿Cómo veo el perfil de un docente?",a:"Hacé clic en el nombre del docente en cualquier publicación para ver su perfil completo con credenciales y publicaciones."},
    {q:"¿Puedo chatear sin estar inscripto?",a:"No, el chat solo está disponible si estás inscripto en la clase/curso del docente, o si el docente aceptó tu oferta."},
    {q:"¿Cómo agrego favoritos?",a:"En cada publicación hay un botón ★ para guardar en favoritos. Podés verlos todos en la sección Favoritos del menú."},
  ];
  const send=async()=>{
    ...
    setLoading(true);
    // Check FAQs first
    const faq=FAQS.find(f=>f.q.toLowerCase().split(" ").some(w=>w.length>4&&q.toLowerCase().includes(w)));
...
export default function App(){
  ...
  const refreshUnread=useCallback(()=>{
    ...
    Promise.all([sb.getMisChats(session.user.email,session.access_token),sb.getOfertasRecibidas(session.user.email,session.access_token)]).then(([msgs,ofertas])=>{
      ...
      setUnread(msgs.filter(m=>m.de_nombre!==session.user.email&&!m.leido&&!(m.publicacion_id===openId&&(m.de_nombre===openOtro||m.para_nombre===openOtro))).length);
  ...
  const closeChat=()=>{chatPostRef.current=null;setChatPost(null);refreshUnread();};
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"66px 13px 22px":"30px 28px",minHeight:"100vh",maxWidth:`calc(100vw - ${SW}px)`}}>
        <div style={{maxWidth:820,margin:"0 auto"}}>
          ...
          {page==="chats"&&<ChatsPage session={session} onOpenChat={openChat}/>}
          {page==="favoritos"&&<FavoritosPage session={session} onOpenDetail={setDetailPost} onOpenChat={openChat} onOpenPerfil={setPerfilEmail}/>}
          {page==="inscripciones"&&<InscripcionesPage session={session} onOpenCurso={setCursoPost} onOpenChat={openChat}/>}
