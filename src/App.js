...
function AuthScreen({onLogin}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Open Sans',sans-serif"}}>
...
export default function App() {
  ...
  if(!session) return (
    <>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}*{box-sizing:border-box;margin:0;padding:0}html,body,#root{background:${C.bg};min-height:100vh;font-family:'Open Sans',sans-serif}`}</style>
      <AuthScreen onLogin={setSession}/>
    </>
  );

  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Open Sans',sans-serif",color:C.text,display:"flex"}}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}*{box-sizing:border-box}html,body,#root{background:${C.bg};min-height:100vh;font-family:'Open Sans',sans-serif}::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:${C.border};border-radius:4px}`}</style>
