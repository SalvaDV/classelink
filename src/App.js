...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil}){
  ...
  const filtered=posts.filter(p=>{
    if(busqueda&&!p.titulo?.toLowerCase().includes(busqueda.toLowerCase())&&!p.descripcion?.toLowerCase().includes(busqueda.toLowerCase()))return false;
    ...
    <div style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      <div style={{position:"sticky",top:0,zIndex:10,background:C.sidebar,borderBottom:`1px solid ${C.border}`,padding:"12px 20px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {esMio&&<button onClick={()=>setShowFinalizar(true)} style={{background:"#4ECB7122",border:"1px solid #4ECB7144",borderRadius:9,color:C.success,padding:"6px 12px",cursor:"pointer",fontSize:12,fontFamily:FONT,fontWeight:600}}>✅ Finalizar</button>}
...
function DetailModal({post,session,onClose,onChat,onOpenCurso,onOpenPerfil}){
  ...
  useEffect(()=>{
    Promise.all([sb.getReseñas(post.id,session.access_token),sb.getReseñasByAutor(post.autor_email,session.access_token),sb.getMisInscripciones(session.user.email,session.access_token)]).then(([pub,usr,ins])=>{setReseñas(pub);setReseñasUsuario(usr);setInscripcion(ins.find(i=>i.publicacion_id===post.id)||null);}).finally(()=>setLoading(false));
...
// ─── PERFIL MODAL — muestra documentos/credenciales del usuario ───────────────
function PerfilModal({autorEmail,session,onClose,onOpenDetail}){
  ...
  return(
    <Modal onClose={onClose} width="min(580px,97vw)">
      <div style={{padding:"20px 22px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18}}>
          <div style={{display:"flex",gap:14,alignItems:"center"}}>
            <div style={{width:54,height:54,borderRadius:"50%",background:perfilColor,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:22,color:"#0D0D0D",fontFamily:FONT,flexShrink:0}}>{nombre[0].toUpperCase()}</div>
            <div><h2 style={{color:C.text,fontSize:18,fontWeight:700,margin:"0 0 4px"}}>{nombre}</h2><div style={{color:C.muted,fontSize:12,marginBottom:5}}>{autorEmail}</div><StarRating val={avg} count={reseñas.length}/></div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:21,cursor:"pointer"}}>×</button>
        </div>
        <div style={{display:"flex",gap:9,marginBottom:18,flexWrap:"wrap"}}>
          <Chip label="PUBLICACIONES" val={`${pubs.length}`}/><Chip label="RESEÑAS" val={`${reseñas.length}`}/>{avg&&<Chip label="PROMEDIO" val={`${avg.toFixed(1)} ★`}/>}
        </div>
        {/* Credenciales/titulos */}
        {docs.length>0&&(
          <div style={{borderTop:`1px solid ${C.border}`,paddingTop:14,marginBottom:14}}>
            <div style={{fontWeight:600,color:C.text,fontSize:13,marginBottom:10}}>🎓 Credenciales</div>
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              {docs.map(d=>(<div key={d.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:11,padding:"10px 14px",display:"flex",gap:10,alignItems:"flex-start"}}>
                <span style={{fontSize:18,flexShrink:0}}>{TIPO_ICON[d.tipo]||"📄"}</span>
                <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:14}}>
          ...
          {loading?<Spinner/>:pubs.length===0?<div style={{color:C.muted,fontSize:13,textAlign:"center",padding:"20px 0"}}>Sin publicaciones activas.</div>:(
            <div style={{display:"flex",flexDirection:"column",gap:9}}>
              {pubs.map(p=>(<div key={p.id} onClick={()=>{onClose();onOpenDetail(p);}} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"11px 14px",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor=C.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
                <div style={{display:"flex",gap:7,alignItems:"center",marginBottom:4,flexWrap:"wrap"}}><Tag tipo={p.tipo}/>{p.verificado&&<VerifiedBadge/>}<span style={{marginLeft:"auto",fontSize:11,color:C.muted}}>{fmt(p.created_at)}</span></div>
...
function MiCuentaPage({session,onOpenDetail,onOpenCurso,onEdit,onNew,onOpenChat,onRefreshOfertas}){
  ...
  const cargar=useCallback(async()=>{
    ...
    try{
      const [p,r,d,ofertasRaw]=await Promise.all([
        sb.getMisPublicaciones(email,session.access_token),
        sb.getReseñasByAutor(email,session.access_token),
        sb.getDocumentos(email,session.access_token).catch(()=>[]),
        sb.getOfertasRecibidas(email,session.access_token)
      ]);
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {/* ── Mis ofertas (📚 Ofrezco clases) ── */}
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h3 style={{color:C.text,fontSize:16,fontWeight:700,margin:0}}>📚 Ofrezco clases <span style={{color:C.muted,fontWeight:400,fontSize:13}}>({ofertas.length})</span></h3>
      ...
      {/* ── Mis búsquedas (🔍 Busco clases) ── */}
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h3 style={{color:C.text,fontSize:16,fontWeight:700,margin:0}}>🔍 Busco clases <span style={{color:C.muted,fontWeight:400,fontSize:13}}>({busquedas.length})</span></h3>
          <Btn onClick={onNew} style={{padding:"6px 12px",fontSize:12}}>+ Nueva</Btn>
        </div>
...
function ChatBotWidget(){
  ...
  const FAQS=[
    ...
    {q:"¿Cómo contacto a un docente?",a:"Desde la card de la publicación o el detalle, presioná el botón '💬 Contactar' para abrir el chat."},
      ...
      {open&&(
        <div style={{position:"absolute",bottom:64,right:0,width:"min(340px,92vw)",background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,boxShadow:"0 8px 32px #0008",display:"flex",flexDirection:"column",maxHeight:460,overflow:"hidden"}}>
          ...
          <div style={{padding:"10px 12px 0",display:"flex",gap:5,flexWrap:"wrap",borderBottom:`1px solid ${C.border}`}}>
            {FAQS.slice(0,3).map((f,i)=>(<button key={i} onClick={()=>{setInput(f.q);}} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,color:C.muted,padding:"4px 9px",fontSize:10,cursor:"pointer",fontFamily:FONT,marginBottom:8}}>{f.q}</button>))}
          </div>
          {/* Messages */}
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"66px 13px 22px":"30px 28px",minHeight:"100vh",maxWidth:`calc(100vw - ${SW}px)`}}>
        <div style={{maxWidth:820,margin:"0 auto"}}>
          ...
          {page==="cuenta"&&<MiCuentaPage key={myPostsKey} session={session} onOpenDetail={setDetailPost} onOpenCurso={setCursoPost} onEdit={p=>{setEditPost(p);setShowForm(true);}} onNew={()=>{setEditPost(null);setShowForm(true);}} onOpenChat={setChatPost} onRefreshOfertas={refreshUnread}/>}
        </div>
      </main>
