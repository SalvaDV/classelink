...
const THEMES={
  dark:{bg:"#fff",surface:"#111",card:"#181818",border:"#242424",accent:"#F5C842",accentDim:"#F5C84215",text:"#F0EDE6",muted:"#666",success:"#4ECB71",danger:"#E05C5C",sidebar:"#0A0A0A",info:"#5CA8E0",purple:"#C85CE0",warn:"#E0955C"},
      ...
      <div style={{background:C.surface,borderRadius:12,padding:"14px 16px",marginBottom:14,border:`1px solid ${C.border}`,boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
        <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
          <div style={{position:"relative",flex:1}}>
            ...
            <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} onKeyDown={e=>e.key==="Escape"&&setBusqueda("")}
              placeholder="Buscar clases, materias o docentes..."
              style={{width:"100%",background:C.bg,border:`1px solid ${busqueda?C.accent:C.border}`,borderRadius:8,padding:"9px 32px 9px 36px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:FONT,transition:"border-color .15s"}}
              onFocus={e=>e.target.style.borderColor=C.accent} onBlur={e=>{if(!busqueda)e.target.style.borderColor=C.border;}}/>
            {busqueda&&<button onClick={()=>setBusqueda("")} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.muted,fontSize:16,cursor:"pointer",lineHeight:1,padding:0}}>×</button>}
          </div>
          <button onClick={()=>setShowBusquedaIA(true)}
            style={{display:"flex",alignItems:"center",gap:5,background:"#7B3FBE12",border:"1px solid #7B3FBE33",borderRadius:8,color:C.purple,padding:"9px 14px",cursor:"pointer",fontFamily:FONT,fontSize:12,fontWeight:600,flexShrink:0,whiteSpace:"nowrap"}}
            onMouseEnter={e=>e.currentTarget.style.background="#7B3FBE20"} onMouseLeave={e=>e.currentTarget.style.background="#7B3FBE12"}>
            ✦ IA
          </button>
          <button onClick={()=>setPanelOpen(v=>!v)}
            style={{display:"flex",alignItems:"center",gap:6,background:panelOpen||hasFilters?C.accentDim:C.bg,border:`1px solid ${panelOpen||hasFilters?C.accent:C.border}`,borderRadius:8,color:panelOpen||hasFilters?C.accent:C.muted,padding:"9px 14px",cursor:"pointer",fontFamily:FONT,fontSize:12,fontWeight:600,flexShrink:0,whiteSpace:"nowrap",transition:"all .12s"}}>
            ≡ Filtros{activeFilters.length>0&&<span style={{background:C.accent,color:"#fff",borderRadius:"50%",width:16,height:16,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,marginLeft:2}}>{activeFilters.length}</span>}
          </button>
        </div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8,alignItems:"center"}}>
            ...
            {ofertasPendientes>0&&<span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:20,background:C.accentDim,color:C.accent,border:`1px solid ${C.accent}33`}}>{ofertasPendientes} oferta{ofertasPendientes!==1?"s":""} nueva{ofertasPendientes!==1?"s":""}</span>}
          ...
          <h3 style={{color:C.text,fontSize:14,fontWeight:600,margin:"0 0 4px",lineHeight:1.35}}>{post.titulo}</h3>
          <p style={{color:C.muted,fontSize:13,margin:0,lineHeight:1.5}}>{post.descripcion?.slice(0,100)}{post.descripcion?.length>100?"...":""}</p>
          {/* Métricas */}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0,minWidth:88}}>
          {post.tipo==="oferta"&&(
            <button onClick={()=>onOpenCurso(pendienteValidacion?{...post,_openValidacion:true}:post)}
              style={{background:pendienteValidacion?C.accent:C.accentDim,border:`1px solid ${C.accent}${pendienteValidacion?"":"44"}`,borderRadius:6,color:pendienteValidacion?"#fff":C.accent,padding:"6px 8px",cursor:"pointer",fontSize:11,fontWeight:600,fontFamily:FONT,textAlign:"center"}}>
              {pendienteValidacion?"⏳ Validar":"Contenido"}
            </button>
          )}
          {!finalizado&&(
            <button onClick={()=>onEdit(post)}
              style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"6px 8px",cursor:"pointer",fontSize:11,fontWeight:500,fontFamily:FONT,textAlign:"center",transition:"border-color .12s"}}
              onMouseEnter={e=>e.currentTarget.style.borderColor=C.accent}
              onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
              Editar
            </button>
          )}
          {!finalizado&&!pendienteValidacion&&(
            <button onClick={()=>onToggle(post)} disabled={toggling===post.id}
              style={{background:activo?C.warn+"12":C.success+"12",border:`1px solid ${activo?C.warn+"40":C.success+"40"}`,borderRadius:6,color:activo?C.warn:C.success,padding:"6px 8px",cursor:"pointer",fontSize:11,fontWeight:500,fontFamily:FONT,textAlign:"center",opacity:toggling===post.id?.5:1}}>
              {toggling===post.id?"...":(activo?"Pausar":"Activar")}
            </button>
          )}
          <button onClick={handleClickEliminar}
            style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.muted,padding:"6px 8px",cursor:"pointer",fontSize:11,fontFamily:FONT,textAlign:"center",transition:"all .12s"}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor=C.danger;e.currentTarget.style.color=C.danger;e.currentTarget.style.background=C.danger+"08";}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.muted;e.currentTarget.style.background=C.bg;}}>
            Eliminar
          </button>
        </div>
      </div>
      {confirmDelete&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:FONT}} onClick={()=>setConfirmDelete(false)}>
          <div style={{background:C.surface,borderRadius:12,padding:"28px",width:"min(400px,92vw)",textAlign:"center",boxShadow:"0 8px 40px rgba(0,0,0,.15)"}} onClick={e=>e.stopPropagation()}>
            ...
            <p style={{color:C.muted,fontSize:13,lineHeight:1.6,margin:"0 0 22px"}}>Se eliminará <strong style={{color:C.text}}>"{post.titulo}"</strong> permanentemente. Esta acción no se puede deshacer.</p>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setConfirmDelete(false)} style={{flex:1,background:"transparent",border:`1px solid ${C.border}`,borderRadius:20,color:C.muted,padding:"10px",cursor:"pointer",fontSize:13,fontFamily:FONT,fontWeight:600}}>Cancelar</button>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:18,width:"min(500px,98vw)",height:"min(680px,90vh)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"12px 14px",display:"flex",flexDirection:"column",gap:6}}>
          {loading?<Spinner/>:msgs.length===0?<div style={{color:C.muted,textAlign:"center",padding:28,fontSize:13}}>Empezá la conversación 👋</div>
            :msgs.map((m,i)=>(<div key={i} style={{display:"flex",justifyContent:m.de_nombre===miEmail?"flex-end":"flex-start"}}><div style={{background:m.de_nombre===miEmail?C.accent:C.card,color:m.de_nombre===miEmail?"#fff":C.text,padding:"8px 12px",borderRadius:13,maxWidth:"78%",fontSize:13,lineHeight:1.5}}>{m.texto}</div></div>))}
          <div ref={bottomRef}/>
        </div>
      ...
      {fase==="habilidades"&&(
        <div>
          ...
          <button onClick={siguienteDesdeHabilidades} disabled={generando||skills.length<2}
            style={{width:"100%",background:C.accent,border:"none",borderRadius:9,color:"#fff",padding:"11px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT,marginTop:8,opacity:skills.length<2?0.5:1}}>
          ...
          {generando?<Spinner small/>:(
            <>
              ...
              <button
                ...
                style={{width:"100%",background:fase==="examen_final"?C.success:C.accent,border:"none",borderRadius:9,color:"#fff",padding:"11px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT,opacity:selecActual.size===0?0.5:1}}>
    ...
    <Modal onClose={onClose} width="min(660px,97vw)">
      <div style={{padding:"19px 21px"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:13}}>
          ...
          {/* ── Chat ── */}
          {!esMio&&puedeChat&&<button onClick={()=>{onClose();onChat(post);}} style={{background:C.accent,color:"#fff",border:"none",borderRadius:11,padding:"8px 16px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT}}>Chatear</button>}
    ...
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:18,padding:"22px 24px",marginBottom:20}}>
        <div style={{display:"flex",gap:16,alignItems:"flex-start",marginBottom:16}}>
          <div style={{position:"relative"}}>
            <div style={{width:60,height:60,borderRadius:"50%",background:currentColor,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:24,color:"#fff",fontFamily:FONT,flexShrink:0}}>{nombre[0].toUpperCase()}</div>
          </div>
          <div style={{flex:1}}>
        ...
        {editingPerfil&&(
          <div style={{background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              ...
              }} disabled={savingDisplayName} style={{background:C.accent,border:"none",borderRadius:9,color:"#fff",padding:"8px 14px",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:FONT}}>{savingDisplayName?"Guardando...":"Guardar"}</button>
            </div>
                ...
                {[["dark","🌙 Oscuro"],["light","☀️ Claro"]].map(([key,label])=>(
                  <button key={key} onClick={()=>{if(window.__setAppTheme)window.__setAppTheme(key);}}
                    style={{padding:"5px 11px",borderRadius:20,fontSize:11,cursor:"pointer",fontFamily:FONT,
                      ...
                      color:(localStorage.getItem("cl_theme")||"dark")===key?"#fff":C.muted,
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          ...
          <button onClick={()=>setShowAddDoc(v=>!v)} style={{background:C.accentDim,border:`1px solid ${C.accent}44`,borderRadius:8,color:C.accent,padding:"6px 12px",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:FONT}}>+ Agregar</button>
        </div>
        {showAddDoc&&(
          <div style={{background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:14}}>
            <Label>Tipo</Label>
            <div style={{display:"flex",gap:6,marginBottom:9,flexWrap:"wrap"}}>
              {TIPOS_DOC.map(({v,l})=>(<button key={v} onClick={()=>setDocTipo(v)} style={{padding:"6px 10px",borderRadius:8,fontSize:12,cursor:"pointer",background:docTipo===v?C.accent:C.surface,color:docTipo===v?"#fff":C.muted,border:`1px solid ${docTipo===v?"transparent":C.border}`,fontFamily:FONT,fontWeight:600}}>{l}</button>))}
            </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {pubs.filter(p=>p.tipo==="oferta"&&p.activo===false&&p.estado_validacion==="pendiente").map(p=>(
              <div key={p.id} style={{background:C.surface,borderRadius:10,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}>
                ...
                <button onClick={()=>onOpenCurso({...p,_openValidacion:true})} style={{background:C.warn,border:"none",borderRadius:8,color:"#fff",padding:"6px 14px",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:FONT,flexShrink:0}}>
              ...
              <div key={o.id} style={{background:C.surface,border:"1px solid #4ECB7133",borderRadius:12,padding:"13px 15px"}}>
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0}}>
                    <button onClick={()=>setEspacioModal({...o,_rol:"alumno"})} style={{background:C.accent,border:"none",borderRadius:8,color:"#fff",padding:"6px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>Ver espacio →</button>
                ...
                <div key={o.id} onClick={()=>{if(esNovedad)marcarVista(o);}} style={{background:C.surface,border:`1px solid ${borderColor}`,borderRadius:12,padding:"11px 14px",display:"flex",gap:10,alignItems:"center",position:"relative",overflow:"hidden",cursor:esNovedad?"pointer":"default"}}>
                  ...
                  {o.estado==="aceptada"&&(
                    <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0}} onClick={e=>{e.stopPropagation();marcarVista(o);}}>
                      <button onClick={()=>{marcarVista(o);setEspacioModal({...o,_rol:"docente"});}} style={{background:C.accent,border:"none",borderRadius:8,color:"#fff",padding:"6px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>Ver espacio →</button>
        ...
        <div style={{position:"absolute",bottom:64,right:0,width:"min(340px,92vw)",background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,boxShadow:"0 8px 32px #0008",display:"flex",flexDirection:"column",maxHeight:460,overflow:"hidden"}}>
          ...
          <div style={{background:C.accent,borderRadius:"20px 20px 0 0",padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:9}}>
              <span style={{fontSize:20}}>🎓</span>
              <div><div style={{fontWeight:700,color:"#fff",fontSize:13}}>Soporte ClasseLink</div><div style={{fontSize:11,color:"#0D0D0D99"}}>Responde al instante</div></div>
            </div>
