...
function QuizViewer({item,session,esMio,esAyudante,onReabrir}){
  ...
  useEffect(()=>{
    if(esMio||esAyudante){
      ...
      sb.getQuizEntregas(item.id,session.access_token).then(e=>setEntregas(e||[])).finally(()=>setLoadingEntregas(false));
      setLoadingEntrega(false);
    } else {
      sb.getMiEntregaQuiz(item.id,session.user.email,session.access_token).then(e=>{
        setMiEntrega(e);
  ...
  const enviarEntregable=async()=>{
    if(!textoEntrega.trim())return;
    setEnviando(true);
    try{
      const r=await sb.insertEntregaQuiz({quiz_id:item.id,publicacion_id:item.publicacion_id,alumno_email:session.user.email,tipo:"entregable",texto_entrega:textoEntrega.trim()},session.access_token);
      setMiEntrega(r?.[0]);
    }catch(e){alert(e.message);}finally{setEnviando(false);}
  };
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {entregas.map(e=>(
              <div key={e.id} style={{background:C.card,border:`1px solid ${e.corregido||e.resultado_json?C.success:C.border}`,borderRadius:10,padding:"10px 13px"}}>
                ...
                {e.tipo==="entregable"&&(
                  <>
                    <p style={{color:C.muted,fontSize:12,margin:"0 0 8px",lineHeight:1.5,background:C.surface,borderRadius:7,padding:"7px 10px"}}>{e.texto_entrega}</p>
                    {!e.corregido&&(
    ...
    <div style={{marginTop:8}}>
      {tipo_quiz==="multiple"?(
        ...
        <div>
          ...
          <textarea value={textoEntrega} onChange={e=>setTextoEntrega(e.target.value)} placeholder="Escribí tu entrega acá..." style={{...iS,minHeight:100,resize:"vertical",marginBottom:8}}/>
          <button onClick={enviarEntregable} disabled={enviando||!textoEntrega.trim()}
            style={{background:C.accent,border:"none",borderRadius:9,color:"#0D0D0D",padding:"9px 20px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT,width:"100%",opacity:!textoEntrega.trim()?0.5:1}}>
                ...
                {contenido.map((c,i)=>{
                  ...
                  if(c.tipo==="quiz"){
                    ...
                    return(<div key={c.id} style={{background:C.card,border:`1px solid ${C.purple}33`,borderRadius:12,padding:"12px 15px"}}>
                      ...
                      {tieneAcceso&&<QuizViewer item={c} session={session} esMio={esMio} esAyudante={esAyudante} onReabrir={reabrirQuiz}/>}
...
function VerificacionIA({titulo,materia,descripcion,onVerificado,token}){
  ...
  useEffect(()=>{if(!titulo||!materia)return;setEstado("cargando");setRespuesta("");setPregunta("");setFeedback("");sb.verificarConIA(titulo,materia,descripcion||"","",token).then(r=>{setPregunta(r.pregunta||"Contá tu experiencia.");setEstado("esperando");}).catch(()=>{setPregunta("Contá brevemente tu experiencia enseñando este tema.");setEstado("esperando");});},[titulo,materia]); // eslint-disable-line
    ...
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        {tipo==="oferta"&&(<>
          ...
          {titulo&&materia&&!verificado&&<VerificacionIA titulo={titulo} materia={materia} descripcion={descripcion} onVerificado={(v)=>{setVerificado(v!==false);}} token={session?.access_token}/>}
