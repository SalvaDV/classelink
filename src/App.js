...
function OfertarBtn({post,session}){
  ...
  return(
    <>
      {/* IMPORTANT: stopPropagation prevents triggering parent card click */}
      <button onClick={e=>{e.stopPropagation();e.preventDefault();setOpen(true);}} style={{background:"#5CA8E022",border:"1px solid #5CA8E044",borderRadius:10,color:C.info,padding:"6px 12px",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:FONT,position:"relative",zIndex:2}}>🙋 Ofertar mis clases</button>
      {open&&(
        <PortalOverlay onClose={()=>setOpen(false)}>
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:18,width:"min(430px,95vw)",padding:"22px"}}>
...
function InscripcionesPage({session,onOpenCurso}){
  ...
  useEffect(()=>{sb.getMisInscripciones(session.user.email,session.access_token).then(async ins=>{setInscripciones(ins);const ids=[...new Set(ins.map(i=>i.publicacion_id))];if(ids.length>0){const pubs=await sb.getPublicaciones({},session.access_token);const map={};pubs.filter(p=>ids.includes(p.id)).forEach(p=>map[p.id]=p);setPosts(map);}}).finally(()=>setLoading(false));},[session]);
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {loading?<Spinner/>:inscripciones.length===0?(<div style={{textAlign:"center",padding:"60px 0"}}><div style={{fontSize:40,marginBottom:12}}>📚</div><p style={{color:C.muted,fontSize:13}}>No estás inscripto en ningún curso.</p></div>):(
        <div style={{display:"grid",gap:10}}>
          {inscripciones.map(ins=>{const p=posts[ins.publicacion_id];if(!p)return null;return(
            <div key={ins.id} onClick={()=>onOpenCurso(p)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 18px",cursor:"pointer",display:"flex",gap:13,alignItems:"center"}} onMouseEnter={e=>e.currentTarget.style.borderColor=C.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
              <div style={{width:42,height:42,borderRadius:11,background:ins.clase_finalizada?"#5CA8E015":C.accentDim,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>
                {ins.clase_finalizada?"✅":"📦"}
              </div>
              <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:C.text,fontSize:14,marginBottom:2}}>{p.titulo}</div><div style={{fontSize:12,color:C.muted}}>{p.materia} · {p.autor_nombre||p.autor_email?.split("@")[0]}</div></div>
              {ins.clase_finalizada?<span style={{fontSize:11,color:C.info,fontWeight:600,flexShrink:0}}>✓ Finalizada</span>:<div style={{fontSize:11,color:C.muted,flexShrink:0}}>Inscripto {fmt(ins.created_at)}</div>}
            </div>
          );})}
        </div>
      )}
    ...
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        {tipo==="oferta"&&(<>
          ...
          {tipo==="oferta"&&titulo&&materia&&!verificado&&<VerificacionIA titulo={titulo} materia={materia} onVerificado={()=>setVerificado(true)} onOmitir={()=>{/* no hace nada: publicar sin verificar */}}/>}
