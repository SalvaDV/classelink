...
// ─── CONTENIDO OFERTA PANEL ───────────────────────────────────────────────────
// Panel de contenido para una oferta aceptada (ofertante sube, alumno lee)
function ContenidoOfertaPanel({ofertaId, session, esMio}){
  ...
  useEffect(()=>{
    sb.getContenido(ofertaId,session.access_token)
      .then(r=>setContenido(r||[]))
      .catch(()=>{})
      .finally(()=>setLoading(false));
  },[ofertaId,session.access_token]); // eslint-disable-line

  const agregar=async()=>{
    if(!nuevoTitulo.trim())return;
    setSaving(true);
    try{
      const item=await sb.insertContenido({
        publicacion_id:ofertaId,tipo:nuevoTipo,
        titulo:nuevoTitulo.trim(),body:nuevoBody.trim()||null,orden:contenido.length+1
      },session.access_token);
      setContenido(p=>[...p,...(Array.isArray(item)?item:[item])]);
  ...
  const iS={width:"100%",background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:9,
    padding:"8px 11px",color:C.text,fontSize:12,outline:"none",fontFamily:FONT,boxSizing:"border-box",marginBottom:7};

  return(
    <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:4}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
        <span style={{fontWeight:600,color:C.text,fontSize:12}}>Material de clases{contenido.length>0&&<span style={{color:C.muted,fontWeight:400}}> ({contenido.length})</span>}</span>
        {esMio&&<button onClick={()=>setShowAdd(v=>!v)} style={{background:C.accentDim,border:"1px solid #F5C84244",borderRadius:7,color:C.accent,padding:"3px 9px",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:FONT}}>{showAdd?"Cancelar":"+ Agregar"}</button>}
      </div>
      {esMio&&showAdd&&(
        <div style={{background:"#111",border:`1px solid ${C.border}`,borderRadius:11,padding:"12px",marginBottom:10}}>
          <div style={{display:"flex",gap:5,marginBottom:8,flexWrap:"wrap"}}>
            {[["texto","Texto"],["video","Video"],["link","Link"],["archivo","Archivo"],["aviso","Aviso"],["tarea","Tarea"]].map(([v,l])=>(
              <button key={v} onClick={()=>setNuevoTipo(v)} style={{padding:"3px 9px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:FONT,
                background:nuevoTipo===v?C.accent:C.card,color:nuevoTipo===v?"#0D0D0D":C.muted,border:`1px solid ${nuevoTipo===v?"transparent":C.border}`}}>{l}</button>
            ))}
          </div>
          <input value={nuevoTitulo} onChange={e=>setNuevoTitulo(e.target.value)} placeholder="Título" style={iS}/>
          <textarea value={nuevoBody} onChange={e=>setNuevoBody(e.target.value)}
            placeholder={nuevoTipo==="link"?"URL del enlace":nuevoTipo==="video"?"URL del video":"Descripción (opcional)"}
            style={{...iS,minHeight:55,resize:"vertical"}}/>
          <Btn onClick={agregar} disabled={saving||!nuevoTitulo.trim()} style={{width:"100%",padding:"7px"}}>{saving?"Guardando...":"Agregar"}</Btn>
        </div>
      )}
      {loading?<Spinner small/>:contenido.length===0?(
        <div style={{color:C.muted,fontSize:11,fontStyle:"italic",textAlign:"center",padding:"8px 0"}}>
          {esMio?"Aún no agregaste material.":"El docente aún no subió material."}
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {contenido.map(item=>(
            <div key={item.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:9,padding:"9px 12px",display:"flex",gap:9,alignItems:"flex-start"}}>
              <div style={{width:26,height:26,borderRadius:6,background:C.accentDim,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:C.accent,fontWeight:700,flexShrink:0}}>
                {TIPO_ICON[item.tipo]||"·"}
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:600,color:C.text,fontSize:12}}>{item.titulo}</div>
                {item.body&&(item.tipo==="link"||item.tipo==="video"?(
                  <a href={item.body} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:C.info,wordBreak:"break-all"}}>{item.body}</a>
                ):(
                  <div style={{fontSize:11,color:C.muted,marginTop:2,lineHeight:1.4}}>{item.body}</div>
                ))}
              </div>
              {esMio&&<button onClick={()=>eliminar(item.id)} style={{background:"none",border:"none",color:C.danger,fontSize:14,cursor:"pointer",flexShrink:0,padding:"0 2px"}}>×</button>}
            </div>
          ))}
        </div>
      )}
              ...
              <div key={o.id} style={{background:C.surface,border:"1px solid #4ECB7133",borderRadius:12,padding:"13px 15px"}}>
                <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:5,alignItems:"flex-end"}}>
                    <button onClick={()=>onOpenChat&&onOpenChat({id:o.busqueda_id,autor_email:o.ofertante_email,titulo:o.busqueda_titulo,autor_nombre:o.ofertante_nombre||o.ofertante_email?.split("@")[0]})}
                      style={{background:C.accent,border:"none",borderRadius:8,color:"#0D0D0D",padding:"5px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700,flexShrink:0}}>Contactar</button>
                    <button onClick={async()=>{try{await sb.updateOfertaBusq(o.id,{finalizada_cuenta:true},session.access_token);setOfertasAceptRec(prev=>prev.map(x=>x.id===o.id?{...x,finalizada_cuenta:true}:x));}catch(e){alert(e.message);}}}
                      style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.muted,padding:"4px 10px",cursor:"pointer",fontSize:11,fontFamily:FONT,flexShrink:0}}>Finalizar</button>
                  </div>
                </div>
                <ContenidoOfertaPanel ofertaId={o.id} session={session} esMio={false}/>
              </div>
            ))}
          </div>
        </div>
      ...
      {(()=>{
        ...
        return(
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>(
                <div key={o.id} style={{background:C.surface,border:`1px solid ${o.estado==="rechazada"?"#E05C5C33":o.estado==="aceptada"?"#4ECB7133":C.border}`,borderRadius:12,padding:"13px 15px"}}>
                  <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:o.estado==="aceptada"?10:0}}>
                    <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:5,alignItems:"flex-end"}}>
                      ...
                      {o.estado==="aceptada"&&<button onClick={()=>onOpenChat&&onOpenChat({id:o.busqueda_id,autor_email:o.busqueda_autor_email,titulo:o.busqueda_titulo,autor_nombre:o.busqueda_autor_email?.split("@")[0]})}
                        style={{background:C.accent,border:"none",borderRadius:8,color:"#0D0D0D",padding:"5px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700,flexShrink:0}}>Contactar</button>}
                      {o.estado==="aceptada"&&<button onClick={async()=>{try{await sb.updateOfertaBusq(o.id,{finalizada_cuenta:true},session.access_token);setMisOfertasEnv(prev=>prev.map(x=>x.id===o.id?{...x,finalizada_cuenta:true}:x));}catch(e){alert(e.message);}}}
                        style={{background:"none",border:"1px solid #4ECB7133",borderRadius:8,color:C.success,padding:"4px 10px",cursor:"pointer",fontSize:11,fontFamily:FONT,flexShrink:0}}>Finalizar</button>}
                    </div>
                  </div>
                  {o.estado==="aceptada"&&<ContenidoOfertaPanel ofertaId={o.id} session={session} esMio={true}/>}
                </div>
              ))}
            </div>
          </div>
