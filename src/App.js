...
function OfertarBtn({post,session}){
  ...
  const enviar=async()=>{
    ...
    try{
      const payload={busqueda_id:post.id,busqueda_autor_email:post.autor_email,ofertante_id:session.user.id,ofertante_email:session.user.email,ofertante_nombre:sb.getDisplayName(session.user.email),precio:precio?parseFloat(precio):null,precio_tipo:tipo,mensaje:msg,leida:false};
...
function ChatModal({post,session,onClose,onUnreadChange}){
  ...
  const send=async()=>{if(!input.trim())return;const txt=input;setInput("");try{await sb.insertMensaje({publicacion_id:post.id,de_usuario:session.user.id,para_usuario:null,de_nombre:miEmail,para_nombre:otroEmail,texto:txt,leido:false,pub_titulo:post.titulo},session.access_token);cargar();}catch(e){alert("Error al enviar: "+e.message);}};
    ...
    <div  ref={pageRef} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      <div style={{position:"sticky",top:0,zIndex:10,background:C.sidebar,borderBottom:`1px solid ${C.border}`,padding:"12px 20px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          {esMio&&!localFinalizado&&<button onClick={()=>setShowCerrarInsc(true)} style={{background:"#E0955C15",border:"1px solid #E0955C33",borderRadius:9,color:C.warn,padding:"6px 12px",cursor:"pointer",fontSize:12,fontFamily:FONT,fontWeight:600}}>Cerrar inscripciones</button>}
    ...
    <Modal onClose={onClose} width="min(660px,97vw)">
      <div style={{padding:"19px 21px"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:13}}>
          ...
          {post.tipo==="oferta"&&(esMio||puedeChat)&&<button onClick={()=>{onClose();onOpenCurso(post);}} style={{background:"#4ECB7122",color:C.success,border:"1px solid #4ECB7144",borderRadius:11,padding:"8px 16px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT}}>📚 Ver curso</button>}
        ...
        {editingPerfil&&(
          <div style={{background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              ...
              <button onClick={async()=>{setSavingDisplayName(true);sb.setDisplayName(email,displayName.trim()||email.split("@")[0]);setSavingDisplayName(false);alert("Nombre actualizado.");}} disabled={savingDisplayName} style={{background:C.accent,border:"none",borderRadius:9,color:"#0D0D0D",padding:"8px 14px",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:FONT}}>{savingDisplayName?"...":"Guardar"}</button>
            </div>
              ...
              <div style={{marginBottom:16}}>
                ...
                <div style={{display:"grid",gap:10}}>
                  {ofertas.filter(p=>p.modo==="particular").map(p=>(<div key={p.id}>
                    <MyPostCard post={p} session={session} onEdit={onEdit} onToggle={toggle} onDelete={remove} onOpenCurso={onOpenCurso} toggling={toggling} ofertasPendientes={0} inscriptos={inscritosMap[p.id]}/>
                  </div>))}
                </div>
              </div>
            )}
