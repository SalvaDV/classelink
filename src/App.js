...
// ─── TOAST GLOBAL ─────────────────────────────────────────────────────────────
let _toastCb=null;
const toast=(msg,type="info",dur=3000)=>{if(_toastCb)_toastCb({msg,type,id:Date.now()});};
...
const Spinner=({small})=>(<div style={{display:"flex",justifyContent:"center",padding:small?"4px":"32px 0"}}><div style={{width:small?16:26,height:small?16:26,border:`2px solid ${C.border}`,borderTop:`2px solid ${C.accent}`,borderRight:`2px solid ${C.teal||C.accent}40`,borderRadius:"50%",animation:"spin .6s linear infinite"}}/></div>);
const SkeletonCard=()=>(<div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"16px 18px",fontFamily:FONT}}>
...
const SkeletonList=({n=3})=>(<div style={{display:"grid",gap:11}}>{Array.from({length:n}).map((_,i)=><SkeletonCard key={i}/>)}</div>);
...
const Tag=({tipo})=>(<span style={{fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:20,background:tipo==="oferta"?C.accentDim:"#F59E0B12",color:tipo==="oferta"?C.accent:"#B45309",border:`1px solid ${tipo==="oferta"?C.accent+"40":"#F59E0B40"}`,fontFamily:FONT,letterSpacing:.2}}>{tipo==="oferta"?"Oferta":"Búsqueda"}</span>);
...
const VerifiedBadge=()=>(<span style={{fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,background:"linear-gradient(135deg,#1A6ED812,#2EC4A012)",color:C.info,border:`1px solid ${C.info}40`,fontFamily:FONT,display:"inline-flex",alignItems:"center",gap:3}}><span style={{fontSize:10}}>✓</span> Verificado</span>);
...
const MiniStars=({val,count})=>{if(!val)return null;const v=parseFloat(val);return(<span style={{display:"inline-flex",alignItems:"center",gap:4,background:"linear-gradient(135deg,#F59E0B12,#F5C84212)",border:"1px solid #F59E0B35",borderRadius:20,padding:"3px 9px"}}><span style={{color:"#B45309",fontSize:12}}>★</span><span style={{color:"#B45309",fontSize:12,fontWeight:700}}>{v.toFixed(1)}</span>{count>0&&<span style={{color:C.muted,fontSize:11}}>({count})</span>}</span>);};
...
function Sidebar({page,setPage,session,onLogout,onNewPost,unreadCount,ofertasCount,notifCount,ofertasAceptadasNuevas,mobile,open,onClose,theme,onToggleTheme,onForceRender}){
  ...
  const inner=(
    <div style={{width:224,height:"100%",background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      {(()=>{
        const items=[nombre&&nombre!==session.user.email.split("@")[0],session.avatarUrl,false]; // bio no accesible aquí
        const done=items.filter(Boolean).length;
        const pct=Math.round((done/items.length)*100);
        return pct<100?(
          <div style={{padding:"8px 16px",borderBottom:`1px solid ${C.border}`,background:C.accentDim}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              <span style={{fontSize:10,fontWeight:700,color:C.accent,letterSpacing:.3}}>PERFIL {pct}% COMPLETO</span>
            </div>
            <div style={{height:3,background:C.border,borderRadius:2}}>
              <div style={{height:"100%",width:`${pct}%`,background:LUD.grad,borderRadius:2,transition:"width .5s ease"}}/>
            </div>
          </div>
        ):null;
      })()}
      ...
      <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`,background:`linear-gradient(135deg,${C.accentDim},${C.bg})`}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          {session.avatarUrl
            ?<div style={{width:40,height:40,borderRadius:"50%",overflow:"hidden",flexShrink:0,border:`2px solid ${C.accent}40`}}><img src={session.avatarUrl} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/></div>
...
function PostCard({post,session,onOpenChat,onOpenDetail,onOpenPerfil,avgPub,countPub,avgUser,yaOferte,fueRechazado,isFav,favId,onFavChange}){
  ...
  return(
    <div onClick={()=>onOpenDetail(post)} className="cl-card-anim"
      style={{background:C.surface,border:`1px solid ${fueRechazado?C.danger+"40":C.border}`,borderRadius:10,padding:"16px 18px",cursor:"pointer",transition:"box-shadow .18s,border-color .18s",fontFamily:FONT,borderLeft:esMio?`3px solid ${C.accent}`:fueRechazado?`3px solid ${C.danger}`:undefined}}
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-start",minWidth:0}}>
          ...
          <div style={{minWidth:0,flex:1}}>
            <button onClick={e=>{e.stopPropagation();onOpenPerfil(post.autor_email);}}
              ...
              {nombre}{post.verificado&&<span style={{fontSize:10,background:"#1A6ED810",color:C.info,border:"1px solid #1A6ED830",borderRadius:20,padding:"1px 5px",marginLeft:5,fontWeight:700}}>✓</span>}
            </button>
...
function BusquedaIA({onBuscar,iaLoading,onClose}){
  ...
  const SUGERENCIAS=["Clases de guitarra para principiantes","Inglés conversacional avanzado","Matemáticas para ingresantes universitarios","Yoga y meditación online","Programación en Python desde cero","Cocina italiana presencial en Buenos Aires"];
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"80px 16px 16px",fontFamily:FONT,backdropFilter:"blur(4px)",animation:"fadeIn .15s ease"}} onClick={onClose}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,width:"min(600px,98vw)",boxShadow:"0 24px 64px rgba(0,0,0,.3)",overflow:"hidden",animation:"fadeUp .2s ease"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"16px 20px"}}>
          <div style={{fontSize:11,fontWeight:600,color:C.muted,letterSpacing:.5,marginBottom:12}}>SUGERENCIAS</div>
...
function MPCheckoutBtn({post,session,onInscripcionOk}){
  const [estado,setEstado]=useState("idle");// "idle"|"loading"|"disabled"

  // Exponer la función pagar para que InscribirseBtn la pueda llamar
  useEffect(()=>{window._mpCheckoutRef=pagar;return()=>{window._mpCheckoutRef=null;};},[]);// eslint-disable-line
  const pagar=async()=>{
    ...
    try{
      ...
      if(result.disabled){
        // MP no está habilitado aún — mostrar info
        toast("El pago online estará disponible pronto. Coordiná directamente con el docente por ahora 🤝","info",5000);
        setEstado("idle");
        return;
      }

      // Guardar en localStorage para recuperar el contexto al volver
      ...
      toast("No se pudo iniciar el pago: "+err.message,"error",4000);
      setEstado("idle");
    }
  };
  ...
  return(
    <div>
      <button onClick={pagar} disabled={estado==="loading"}
        style={{width:"100%",background:estado==="loading"?"#ccc":"linear-gradient(135deg,#009EE3,#0070BA)",border:"none",borderRadius:20,color:"#fff",padding:"13px",fontWeight:700,fontSize:14,cursor:estado==="loading"?"default":"pointer",fontFamily:FONT,display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 4px 14px rgba(0,158,227,.3)",transition:"opacity .15s,background .2s"}}
        onMouseEnter={e=>{if(estado!=="loading")e.currentTarget.style.opacity=".88";}}
        onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
        {estado==="loading"?"Procesando…":"Pagar con Mercado Pago"}
      </button>
      <div style={{textAlign:"center",fontSize:10,color:C.muted,marginTop:5,display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
        <span>🔒</span> Pago seguro · Hasta 12 cuotas · Sin comisión Luderis
      </div>
    </div>
  );
}
...
function InscribirseBtn({post,session,onDone}){
  ...
  const inscribirse=async()=>{
    if(tienePrecio){
      // Tiene precio → ir al checkout de MP
      if(window._mpCheckoutRef)window._mpCheckoutRef();
      return;
    }
    setLoading(true);
    try{
      ...
      // Email al docente avisando que tiene un nuevo alumno
      const alumnoNombreInsc=sb.getDisplayName(session.user.email)||session.user.email.split("@")[0];
      sb.sendEmail("nueva_inscripcion",post.autor_email,{pub_titulo:post.titulo,alumno_nombre:alumnoNombreInsc},session.access_token).catch(()=>{});
      // Email de comprobante al alumno
      ...
      setOk(true);toast("¡Inscripción exitosa! Ya tenés acceso","success",4000);
      setTimeout(()=>onDone(),700);
    }catch(e){
      if(e.message?.includes("uq_inscripcion")){onDone();}
      else{toast("Error al inscribirse: "+e.message,"error");setLoading(false);}
  ...
  return(
    <button onClick={inscribirse} disabled={loading}
      style={{width:"100%",background:C.bg,color:C.muted,border:`1px solid ${C.border}`,borderRadius:20,padding:"11px",fontWeight:600,fontSize:13,cursor:"pointer",fontFamily:FONT,display:"flex",alignItems:"center",justifyContent:"center",gap:6,transition:"all .15s"}}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=C.accent;e.currentTarget.style.color=C.accent;}}
      onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.muted;}}>
      {loading?"⟳ ...":"Inscribirme sin pago (temporal)"}
    </button>
  );
}
                ...
                {loading?(
                  ...
                  <>
                    ...
                    {post.tipo==="oferta"&&!esMio&&!esAyudante&&!inscripcion&&!post.finalizado&&!post.inscripciones_cerradas&&(
                      <div style={{display:"flex",flexDirection:"column",gap:8}}>
                        {/* Botón principal: Inscribirme con MP (azul — el que quedará) */}
                        <MPCheckoutBtn post={post} session={session} onInscripcionOk={()=>{onClose();onOpenCurso(post);}}/>
                        {/* Botón temporal: inscripción directa sin pago */}
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    if(tipo==="oferta"&&(!precio||parseFloat(precio)<=0)){setErr("El precio es obligatorio para clases y cursos");return;}
    setSaving(true);setErr("");
    try{
      ...
      // ── Verificar alertas de otros usuarios que coincidan con esta publicación ──
      if(savedPub&&!editing){
        sb.dispararAlertas(savedPub,session.access_token).catch(()=>{});
      }
      ...
      {loading?<Spinner/>:alertas.length===0?(
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {alertas.map(a=>{
            ...
            return(
              <div key={a.id} style={{background:C.surface,border:`1px solid ${a.activa?C.accent+"33":C.border}`,borderRadius:12,padding:"14px 16px",display:"flex",gap:12,alignItems:"flex-start",opacity:a.activa?1:0.6,transition:"all .2s"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:12}}>
                <div>
                  ...
                  <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:12}}>
                    <div style={{width:64,height:64,borderRadius:"50%",overflow:"hidden",flexShrink:0,border:`2px solid ${C.border}`,background:C.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {avatarUrl
...
// Permite crear alertas con IA que notifican por email cuando aparece una pub similar
function AlertasTab({session}){
  const [alertas,setAlertas]=useState([]);
  const [loading,setLoading]=useState(true);
  ...
  const crear=async()=>{
    ...
    try{
      ...
      setDescripcion("");setCreando(false);
      toast("Alerta creada. Te avisaremos cuando aparezca algo que coincida ✓","success",4000);
      cargar();
    ...
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20,flexWrap:"wrap",gap:12}}>
        <div>
          <div style={{fontWeight:700,color:C.text,fontSize:16,marginBottom:4}}>Alertas de publicaciones</div>
          <div style={{fontSize:13,color:C.muted,lineHeight:1.5}}>
            Describí lo que estás buscando y te avisamos por email cuando aparezca una publicación que coincida.
          </div>
        </div>
        {!creando&&(
          <button onClick={()=>{setCreando(true);setTimeout(()=>inputRef.current?.focus(),80);}}
            style={{background:LUD.grad,border:"none",borderRadius:20,color:"#fff",padding:"10px 20px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT,boxShadow:"0 4px 12px rgba(26,110,216,.25)",flexShrink:0}}>
            + Nueva alerta
          </button>
        )}
      </div>
      ...
      {/* Formulario nueva alerta */}
      {creando&&(
        <div style={{background:C.surface,border:`2px solid ${C.accent}`,borderRadius:14,padding:"20px",marginBottom:20,animation:"fadeUp .15s ease"}}>
          <div style={{fontWeight:700,color:C.text,fontSize:14,marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:18}}>✦</span> Nueva alerta con IA
          </div>
          <div style={{fontSize:12,color:C.muted,marginBottom:10,lineHeight:1.5}}>
            Describí en tus propias palabras qué tipo de clase o docente estás buscando. La IA va a comparar cada nueva publicación con tu descripción.
          </div>
          <textarea ref={inputRef} value={descripcion} onChange={e=>setDescripcion(e.target.value)}
            placeholder='Ej: "Clases de guitarra eléctrica para adultos, online, preferentemente con experiencia en rock"'
            style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",color:C.text,fontSize:14,outline:"none",resize:"vertical",minHeight:90,boxSizing:"border-box",fontFamily:FONT,lineHeight:1.6}}/>
          <div style={{display:"flex",gap:8,marginTop:12,justifyContent:"flex-end"}}>
            <button onClick={()=>{setCreando(false);setDescripcion("");}}
              style={{background:"none",border:`1px solid ${C.border}`,borderRadius:20,color:C.muted,padding:"8px 18px",cursor:"pointer",fontFamily:FONT,fontSize:13}}>
              Cancelar
            </button>
            <button onClick={crear} disabled={!descripcion.trim()||saving}
              style={{background:descripcion.trim()&&!saving?LUD.grad:"#ccc",border:"none",borderRadius:20,color:"#fff",padding:"8px 20px",fontWeight:700,fontSize:13,cursor:descripcion.trim()&&!saving?"pointer":"default",fontFamily:FONT,transition:"background .15s"}}>
      ...
      {/* Lista de alertas */}
      {loading?<Spinner/>:alertas.length===0&&!creando?(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"48px 24px",textAlign:"center"}}>
          <div style={{fontSize:40,marginBottom:16}}>🔔</div>
          <div style={{fontWeight:700,color:C.text,fontSize:16,marginBottom:8}}>Sin alertas todavía</div>
          <div style={{color:C.muted,fontSize:13,marginBottom:20,maxWidth:360,margin:"0 auto 20px"}}>
            Creá una alerta y te avisamos por email cuando aparezca una publicación que coincida con lo que buscás.
          </div>
          <button onClick={()=>{setCreando(true);setTimeout(()=>inputRef.current?.focus(),80);}}
            style={{background:LUD.grad,border:"none",borderRadius:20,color:"#fff",padding:"10px 24px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT}}>
            Crear mi primera alerta
          </button>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {alertas.map(a=>(
            <div key={a.id} style={{background:C.surface,border:`1px solid ${a.activa?C.accent+"40":C.border}`,borderRadius:12,padding:"16px 18px",display:"flex",gap:14,alignItems:"flex-start",transition:"border-color .15s"}}>
              {/* Toggle activa */}
              <button onClick={()=>toggleActiva(a)} title={a.activa?"Pausar alerta":"Activar alerta"}
                style={{width:36,height:20,borderRadius:10,background:a.activa?C.accent:C.border,border:"none",cursor:"pointer",flexShrink:0,position:"relative",transition:"background .2s",marginTop:2}}>
                <span style={{position:"absolute",top:2,left:a.activa?18:2,width:16,height:16,borderRadius:"50%",background:"#fff",transition:"left .2s",boxShadow:"0 1px 4px rgba(0,0,0,.2)"}}/>
              </button>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:14,color:a.activa?C.text:C.muted,lineHeight:1.5,marginBottom:6}}>"{a.descripcion}"</div>
                <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                  <span style={{fontSize:11,color:a.activa?C.success:C.muted,fontWeight:600}}>{a.activa?"● Activa":"○ Pausada"}</span>
                  {a.ultima_vez&&<span style={{fontSize:11,color:C.muted}}>· Última coincidencia: {fmtRel(a.ultima_vez)}</span>}
                  {a.total_matches>0&&<span style={{fontSize:11,color:C.muted}}>· {a.total_matches} coincidencia{a.total_matches!==1?"s":""}</span>}
                  <span style={{fontSize:11,color:C.muted}}>· Creada {fmtRel(a.created_at)}</span>
                </div>
              </div>
              <button onClick={()=>eliminar(a.id)}
                style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:16,flexShrink:0,padding:4,borderRadius:6,transition:"color .15s"}}
      ...
      <div style={{marginTop:24,padding:"14px 18px",background:C.accentDim,border:`1px solid ${C.accent}22`,borderRadius:12}}>
        <div style={{fontSize:12,fontWeight:700,color:C.accent,marginBottom:6}}>✦ ¿Cómo funciona?</div>
        <div style={{fontSize:12,color:C.muted,lineHeight:1.6}}>
          Cada vez que un docente publica una nueva clase o curso, la IA compara la publicación con tus alertas activas. Si coincide, recibís un email al instante con los detalles y un link directo a la publicación.
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      {detailPost&&<DetailModal post={detailPost} session={session} onClose={()=>setDetailPost(null)} onChat={p=>{setDetailPost(null);openChat(p);}} onOpenCurso={p=>{setDetailPost(null);setCursoPost(p);}} onOpenPerfil={setPerfilEmail} onOpenDetail2={p=>{setDetailPost(null);setTimeout(()=>setDetailPost(p),80);}}/>}
