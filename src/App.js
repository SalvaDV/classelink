...
function OfertarBtn({post,session}){
  ...
  const {useEffect:ue}=window.React||{useEffect};
  useEffect(()=>{
    ...
    <Modal onClose={onClose} width="min(660px,97vw)">
      <div style={{padding:"19px 21px"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:13}}>
          ...
          {!esMio&&post.tipo==="busqueda"&&!puedeChat&&miOfertaPendiente&&<span style={{fontSize:12,color:C.warn,fontStyle:"italic",alignSelf:"center",display:"flex",alignItems:"center",gap:5}}>⏳ Esperando que el autor acepte tu oferta</span>}
          {!esMio&&post.tipo==="busqueda"&&!puedeChat&&!miOfertaPendiente&&!esMio&&<span style={{fontSize:12,color:C.muted,fontStyle:"italic",alignSelf:"center"}}>Ofertá tus clases para poder chatear</span>}
