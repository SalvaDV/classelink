...
function OfertasRecibidasModal({post,session,onClose,onContactar}){
  ...
  const responder=async(o,estado)=>{
    ...
    try{
      ...
      if(estado==="aceptada"){
        // Pausar la búsqueda para que no aparezca en Explorar mientras está acordada
        await sb.updatePublicacion(post.id,{activo:false},session.access_token).catch(()=>{});
        // Notificar al docente que su oferta fue aceptada
                ...
                <div key={o.id} style={{background:esNuevaAceptacion?"#4ECB7108":C.surface,border:`1px solid ${esNuevaAceptacion?"#4ECB71":o.estado==="rechazada"?"#E05C5C33":o.estado==="aceptada"?"#4ECB7133":C.border}`,borderRadius:12,padding:"11px 14px",display:"flex",gap:10,alignItems:"center",position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{flex:1,minWidth:0,paddingLeft:6}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:2}}>
                      ...
                      {esNuevaAceptacion&&<span style={{fontSize:10,fontWeight:700,background:"#4ECB7122",color:C.success,border:"1px solid #4ECB7155",borderRadius:20,padding:"2px 8px",flexShrink:0,animation:"pulse 1.5s infinite"}}>🔔 ¡Aceptada!</span>}
                    </div>
