...
- Las preguntas deben ser claras, educativas y de dificultad media`,
      ...
      if(!match)throw new Error("Sin respuesta válida");
      const parsed=JSON.parse(match[0]);
        ...
        <div>
          ...
          <div style={{background:C.card,border:"1px solid #C85CE033",borderRadius:10,padding:"10px 12px",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:7,alignItems:"center"}}>
              ...
              {[3,5,8,10].map(n=>(
                <button key={n} onClick={()=>setCantIA(n)} style={{padding:"4px 10px",borderRadius:20,fontSize:11,cursor:"pointer",fontFamily:FONT,background:cantIA===n?C.purple:"transparent",color:cantIA===n?"#fff":C.muted,border:`1px solid ${cantIA===n?"#C85CE0":C.border}`,fontWeight:cantIA===n?700:400}}>{n}</button>
              ))}
...
// ─── VERIFICACIÓN IA ──────────────────────────────────────────────────────────
function VerificacionIA({titulo,materia,descripcion,onVerificado,token}){
  ...
  const tituloDebounced=useDebounce(titulo,1500);
  const descripcionDebounced=useDebounce(descripcion,1500);
  useEffect(()=>{
    ...
    sb.verificarConIA(tituloDebounced,materia,descripcionDebounced||"","",token)
      .then(r=>{setPregunta(r.pregunta||"Contá tu experiencia.");setEstado("esperando");})
    ...
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        {tipo==="oferta"&&(<>
          ...
          {titulo&&materia&&descripcion.length>=30&&!verificado&&<VerificacionIA titulo={titulo} materia={materia} descripcion={descripcion} onVerificado={(v)=>{setVerificado(v!==false);}} token={session?.access_token}/>}
