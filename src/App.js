...
function InscribirseBtn({post,session,onDone}){
  ...
  const inscribirse=async()=>{
    // El botón temporal SIEMPRE inscribe directo, sin importar el precio
    setLoading(true);
    try{
...
REGLAS (evaluá TODAS):
1. TEMA: ¿El título/descripción/materia de la pub es relevante para lo que busca? Considerá sinónimos y conceptos relacionados. Es el criterio más importante.
2. MODALIDAD: Si el usuario pidió "presencial" y la clase es "virtual" → NO coincide. Si no especificó → cualquiera sirve.
3. UBICACIÓN: Si el usuario mencionó una ciudad y la clase es presencial en otra ciudad → NO coincide. Si la clase es virtual → la ubicación no importa.
4. CERTIFICADO: Si el usuario explícitamente quiere certificado y la pub no lo otorga → NO coincide.
5. NIVEL: Si el usuario mencionó nivel específico y no coincide → penaliza pero no descarta solo por esto.
6. PRECIO: Solo considerar si el usuario mencionó un rango explícito.

REGLA FINAL: El TEMA debe coincidir. Si hay contradicciones duras (modalidad incompatible, ciudad incorrecta en clase presencial, sin certificado cuando se pide) → NO coincide aunque el tema sea perfecto.
Si el usuario NO especificó restricciones → solo el tema importa.

Respondé SOLO con JSON sin markdown: {"match":true,"razon":"frase corta"} o {"match":false,"razon":"por qué no"}`,
                  `ALERTA DEL USUARIO:\n${alertaContexto}\n\nPUBLICACIÓN NUEVA:\n${pubContexto}`,
                ...
                if(match){
                  const pubUrl=`${window.location.origin}${window.location.pathname}?pub=${savedPub.id}`;
