...
function VerificacionIA({titulo,materia,descripcion,onVerificado,onEstadoChange,token}){
  ...
  const evaluar=async()=>{if(!respuesta.trim())return;setEstado("evaluando");try{const r=await sb.verificarConIA(titulo,materia,descripcion||"",respuesta,token);
    ...
    if(r.correcta){setEstado("ok");onVerificado();}
      ...
      // Falló — mostrar feedback 2s y luego generar nueva pregunta
      setEstado("error");
      setTimeout(()=>{
        setEstado("cargando");setRespuesta("");setFeedback("");
        sb.verificarConIA(titulo,materia,descripcion||"","",token)
          .then(r2=>{setPregunta(r2.pregunta||"Contá tu experiencia.");setEstado("esperando");})
          .catch(()=>{setPregunta("Contá brevemente tu experiencia enseñando este tema.");setEstado("esperando");});
      },2500);
    }}catch{setEstado("error");setFeedback("No se pudo evaluar.");}};
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      const data={tipo,materia,titulo,descripcion,autor_id:session.user.id,activo:true,verificado,modo:modoDb,modalidad:modalidadForm||null,nivel:nivel||null};// banner_url omitido hasta agregar columna en DB
