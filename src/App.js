...
function PriceSlider({min,max,valMin,valMax,onChangeMin,onChangeMax}){
  ...
  const sliderS={WebkitAppearance:"none",appearance:"none",width:"100%",height:4,borderRadius:2,outline:"none",cursor:"pointer",background:"transparent"};
  return(
    <div>
      <div style={{position:"relative",height:20,marginBottom:2}}>
        <div style={{position:"absolute",top:"50%",transform:"translateY(-50%)",left:0,right:0,height:4,background:C.border,borderRadius:2}}/>
        <div style={{position:"absolute",top:"50%",transform:"translateY(-50%)",left:`${pct(valMin)}%`,right:`${100-pct(valMax)}%`,height:4,background:C.accent,borderRadius:2}}/>
        <style>{`.ps-thumb::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:${C.accent};cursor:pointer;border:2px solid #0D0D0D;box-shadow:0 0 0 1px ${C.accent}}.ps-thumb::-moz-range-thumb{width:16px;height:16px;border-radius:50%;background:${C.accent};cursor:pointer;border:2px solid #0D0D0D}`}</style>
        <input type="range" min={min} max={max} value={valMin} onChange={e=>{const v=Math.min(Number(e.target.value),valMax-1);onChangeMin(v);}} className="ps-thumb" style={{...sliderS,position:"absolute",top:0,pointerEvents:"all"}}/>
        <input type="range" min={min} max={max} value={valMax} onChange={e=>{const v=Math.max(Number(e.target.value),valMin+1);onChangeMax(v);}} className="ps-thumb" style={{...sliderS,position:"absolute",top:0,pointerEvents:"all"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.muted}}>
        <span>{fmtPrice(valMin)}</span>
        <span>{fmtPrice(valMax)}{valMax===max&&"+"}</span>
      </div>
    </div>
  );
}
...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil}){
  ...
  const [filtroModo,setFiltroModo]=useState("all");
  const [filtroSinc,setFiltroSinc]=useState("all");
  ...
  const cargar=useCallback(async()=>{
    setLoading(true);
    try{
      const d=await sb.getPublicaciones({},session.access_token);
      const activos=d.filter(p=>p.activo!==false&&!p.finalizado);
      setPosts(activos);
      // Rango dinámico de precios
      const precios=activos.filter(p=>p.precio>0).map(p=>Number(p.precio));
      if(precios.length){
        const mn=Math.floor(Math.min(...precios)/100)*100;
        const mx=Math.ceil(Math.max(...precios)/100)*100;
        setPrecioMin(mn);setPrecioMax(mx);setSliderMin(mn);setSliderMax(mx);
      }
  ...
  const filtered=posts.filter(p=>{
    ...
    if(busqueda&&!p.titulo?.toLowerCase().includes(q)&&!p.descripcion?.toLowerCase().includes(q)&&!(p.autor_nombre||p.autor_email?.split("@")[0]||"").toLowerCase().includes(q))return false;
    ...
    if(precioMax>0){
      if(p.precio&&Number(p.precio)<sliderMin)return false;
      if(p.precio&&Number(p.precio)>sliderMax)return false;
    }
    return true;
  });
  ...
  const activeFilters=[
    ...
    (sliderMin>precioMin||sliderMax<precioMax)&&`${fmtPrice(sliderMin)}–${fmtPrice(sliderMax)}`,
  ].filter(Boolean);
  const hasFilters=activeFilters.length>0||busqueda;
  const clearAll=()=>{setFiltroTipo("all");setFiltroModo("all");setFiltroSinc("all");setFiltroMateria("");setSliderMin(precioMin);setSliderMax(precioMax);setBusqueda("");};

  const FSection=({label,children})=>(
    <div style={{marginBottom:20}}>
      <div style={{fontSize:10,fontWeight:700,letterSpacing:1.2,color:C.muted,textTransform:"uppercase",marginBottom:9}}>{label}</div>
      {children}
    </div>
  );
  const FChip=({label,active,onClick})=>(
    <button onClick={onClick} style={{padding:"5px 12px",borderRadius:20,fontSize:12,fontWeight:active?700:400,cursor:"pointer",fontFamily:FONT,background:active?C.accent:C.surface,color:active?"#0D0D0D":C.muted,border:`1px solid ${active?C.accent:C.border}`,transition:"all .15s",marginBottom:5,marginRight:5}}>
      {label}
    </button>
  );
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
        ...
        <button onClick={()=>setPanelOpen(v=>!v)} style={{display:"flex",alignItems:"center",gap:7,background:panelOpen||hasFilters?C.accentDim:C.surface,border:`1px solid ${panelOpen||hasFilters?C.accent:C.border}`,borderRadius:10,color:panelOpen||hasFilters?C.accent:C.muted,padding:"8px 14px",cursor:"pointer",fontFamily:FONT,fontSize:12,fontWeight:600,flexShrink:0}}>
          ⊟ Filtros{activeFilters.length>0&&<span style={{background:C.accent,color:"#0D0D0D",borderRadius:"50%",width:18,height:18,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700}}>{activeFilters.length}</span>}
        </button>
      </div>

      {/* Search */}
      <div style={{position:"relative",marginBottom:12}}>
        <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar por tema, materia o docente..."
          style={{width:"100%",background:C.surface,border:`1px solid ${busqueda?C.accent:C.border}`,borderRadius:11,padding:"10px 36px 10px 14px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:FONT}}/>
        {busqueda&&<button onClick={()=>setBusqueda("")} style={{position:"absolute",right:11,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",padding:0,lineHeight:1}}>×</button>}
      </div>
      ...
      {activeFilters.length>0&&(
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12,alignItems:"center"}}>
          {activeFilters.map(f=>(
            <span key={f} style={{background:C.accentDim,border:"1px solid #F5C84233",borderRadius:20,padding:"3px 10px",fontSize:11,color:C.accent,fontWeight:600}}>{f}</span>
          ))}
          <button onClick={clearAll} style={{background:"none",border:"none",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:FONT,textDecoration:"underline",padding:"0 4px"}}>Limpiar todo</button>
        </div>
      )}
      ...
      <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
        {/* ── PANEL LATERAL ── */}
        {panelOpen&&(
          <div style={{width:220,flexShrink:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"18px 16px",position:"sticky",top:16,maxHeight:"calc(100vh - 100px)",overflowY:"auto"}}>
            ...
            <FSection label="Tipo">
              <div style={{display:"flex",flexWrap:"wrap"}}>
                {[["all","Todos"],["oferta","Ofertas"],["busqueda","Búsquedas"]].map(([v,l])=>(
                  <FChip key={v} label={l} active={filtroTipo===v} onClick={()=>{setFiltroTipo(v);if(v!=="oferta"){setFiltroModo("all");setFiltroSinc("all");}}}/>
                ))}
              </div>
            </FSection>

            {(filtroTipo==="all"||filtroTipo==="oferta")&&(
              <FSection label="Modalidad">
                <div style={{display:"flex",flexWrap:"wrap"}}>
                  {[["all","Todas"],["curso","Cursos"],["particular","Clases part."]].map(([v,l])=>(
                    <FChip key={v} label={l} active={filtroModo===v} onClick={()=>{setFiltroModo(v);if(v!=="curso")setFiltroSinc("all");}}/>
                  ))}
                </div>
              </FSection>
            )}

            {(filtroModo==="curso"||filtroModo==="all")&&(filtroTipo==="all"||filtroTipo==="oferta")&&(
              <FSection label="Sincronismo">
                <div style={{display:"flex",flexWrap:"wrap"}}>
                  {[["all","Todos"],["sinc","Sincrónico"],["asinc","Asincrónico"]].map(([v,l])=>(
                    <FChip key={v} label={l} active={filtroSinc===v} onClick={()=>setFiltroSinc(v)}/>
                  ))}
                </div>
              </FSection>
            )}

            <FSection label="Materia">
              <div style={{display:"flex",flexWrap:"wrap"}}>
                <FChip label="Todas" active={!filtroMateria} onClick={()=>setFiltroMateria("")}/>
                {MATERIAS.map(m=>(
                  <FChip key={m} label={m} active={filtroMateria===m} onClick={()=>setFiltroMateria(filtroMateria===m?"":m)}/>
                ))}
              </div>
            </FSection>

            {precioMax>0&&(
              <FSection label="Precio">
                <PriceSlider min={precioMin} max={precioMax} valMin={sliderMin} valMax={sliderMax} onChangeMin={setSliderMin} onChangeMax={setSliderMax}/>
              </FSection>
            )}
          </div>
        )}
        ...
        <div style={{flex:1,minWidth:0}}>
          {loading?<Spinner/>:filtered.length===0?(
            <div style={{textAlign:"center",color:C.muted,padding:"60px 0",fontSize:13}}>
              <div style={{fontSize:28,marginBottom:12,color:C.border}}>◎</div>
              {hasFilters?"Sin resultados con esos filtros.":"Sin publicaciones aún."}
              {hasFilters&&<div style={{marginTop:8}}><button onClick={clearAll} style={{background:"none",border:"none",color:C.accent,cursor:"pointer",fontFamily:FONT,fontSize:13}}>Limpiar filtros</button></div>}
            </div>
          ):(
...
function InscripcionesPage({session,onOpenCurso,onOpenChat,onMarkNotifsRead}){
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      // Ofertas donde soy ofertante (soy el docente) y fueron aceptadas
      sb.getMisOfertas(miEmail2,session.access_token).catch(()=>[]),
      // Ofertas aceptadas donde soy dueño de la búsqueda (soy el alumno)
      sb.getOfertasAceptadasRecibidas(miEmail2,session.access_token).catch(()=>[]),
    ]).then(([ins,todasPubs,misOfertas,ofertasRecibidas])=>{
      ...
      setPosts(map);
      // Clases acordadas: soy ofertante (aceptada, no finalizada) + soy alumno (aceptada, no finalizada)
      const comoOfertante=(misOfertas||[]).filter(o=>o.estado==="aceptada"&&!o.finalizada_cuenta).map(o=>({...o,_rol:"docente"}));
      const comoAlumno=(ofertasRecibidas||[]).filter(o=>!o.finalizada_cuenta).map(o=>({...o,_rol:"alumno"}));
      setClasesAcordadas([...comoOfertante,...comoAlumno]);
    }).finally(()=>setLoading(false));
  },[session]);
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {inscripciones.length===0&&ayudantePubs.length===0&&!loading&&clasesAcordadas.length===0&&(
        <div style={{textAlign:"center",padding:"60px 0"}}><div style={{fontSize:40,marginBottom:12,color:C.border}}>◎</div><p style={{color:C.muted,fontSize:13}}>No estás inscripto en ningún curso ni clase.</p></div>
      )}

      {/* ── Clases acordadas (ofertas aceptadas) ── */}
        ...
        <div style={{marginTop:inscripciones.length>0||ayudantePubs.length>0?28:0}}>
          ...
          <div style={{display:"grid",gap:9}}>
            {clasesAcordadas.map(o=>{
              const soyDocente=o._rol==="docente";
              const otroNombre=soyDocente?(o.busqueda_autor_nombre||o.busqueda_autor_email?.split("@")[0]):(o.ofertante_nombre||o.ofertante_email?.split("@")[0]);
              return(
                <div key={o.id} onClick={()=>setEspacioActivo(o)}
                  style={{background:C.card,border:"1px solid #4ECB7133",borderRadius:14,padding:"14px 18px",display:"flex",gap:13,alignItems:"center",cursor:"pointer",transition:"border-color .15s"}}
                  onMouseEnter={e=>e.currentTarget.style.borderColor=C.success} onMouseLeave={e=>e.currentTarget.style.borderColor="#4ECB7133"}>
                  <div style={{width:44,height:44,borderRadius:11,background:"#4ECB7115",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,border:"1px solid #4ECB7133",color:C.success,fontWeight:700}}>
                    {soyDocente?"✦":"◈"}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:12,color:C.muted,marginBottom:3}}>{soyDocente?"Alumno":"Docente"}: <span style={{color:C.text,fontWeight:600}}>{otroNombre}</span></div>
                    <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                      ...
                      {soyDocente&&<span style={{fontSize:11,background:"#F5C84215",color:C.accent,border:"1px solid #F5C84233",borderRadius:20,padding:"1px 8px",fontWeight:600}}>Sos el docente</span>}
                      {o.precio&&<span style={{fontSize:11,color:C.muted}}>{fmtPrice(o.precio)}/{o.precio_tipo||"hora"}</span>}
                    </div>
                  </div>
                  <div style={{fontSize:12,color:C.success,fontWeight:700,flexShrink:0}}>Entrar →</div>
                </div>
              );
            })}
          </div>
...
// ─── ESPACIO CLASE MODAL — espacio privado para oferta aceptada ───────────────
// Funciona como CursoPage pero entre dos personas específicas (ofertante + alumno)
// El contenido se guarda en contenido_curso con publicacion_id = oferta.id
function EspacioClaseModal({oferta, session, onClose}){
  // oferta tiene: id, busqueda_titulo, ofertante_email, ofertante_nombre,
  //               busqueda_autor_email, precio, precio_tipo, mensaje
  const miEmail=session.user.email;
  const soyOfertante=oferta.ofertante_email===miEmail;
  const otroEmail=soyOfertante?oferta.busqueda_autor_email:oferta.ofertante_email;
  const otroNombre=soyOfertante?(oferta.busqueda_autor_nombre||otroEmail?.split("@")[0]):(oferta.ofertante_nombre||otroEmail?.split("@")[0]);
  ...
  const addContenido=async()=>{
    ...
    setSavingC(true);
    try{
      const data={publicacion_id:oferta.id,tipo:nuevoTipo,titulo:nuevoTitulo.trim(),orden:contenido.length+1};
      if(nuevoTipo==="link"||nuevoTipo==="video")data.url=nuevoBody.trim();
      else data.texto=nuevoBody.trim()||null;
  ...
  const removeContenido=async(id)=>{
    try{await sb.deleteContenido(id,session.access_token);setContenido(p=>p.filter(x=>x.id!==id));}
    catch(e){alert(e.message);}
  };

  const TIPO_META={
    video:{icon:"▶",color:C.info,label:"Video"},
    archivo:{icon:"↓",color:C.success,label:"Archivo"},
    texto:{icon:"≡",color:C.text,label:"Texto"},
    aviso:{icon:"!",color:C.accent,label:"Aviso"},
    tarea:{icon:"★",color:C.purple,label:"Tarea"},
    link:{icon:"↗",color:C.info,label:"Link"},
  };
  const iS={width:"100%",background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:9,padding:"9px 12px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:FONT,marginBottom:8};
    ...
    <div ref={pageRef} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:10,background:C.sidebar,borderBottom:`1px solid ${C.border}`,padding:"12px 20px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11,color:C.muted}}>{soyOfertante?"Alumno":"Docente"}: {otroNombre}</div>
        </div>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"24px 20px"}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px 20px",marginBottom:20}}>
          <div style={{display:"flex",gap:14,alignItems:"center"}}>
            <Avatar letra={otroNombre[0]} size={46}/>
            <div style={{flex:1}}>
              <div style={{fontWeight:700,color:C.text,fontSize:15,marginBottom:2}}>{otroNombre}</div>
              <div style={{fontSize:12,color:C.muted,marginBottom:6}}>{otroEmail}</div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <span style={{fontSize:11,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"3px 9px",color:C.muted}}>Clase particular</span>
          ...
          {oferta.mensaje&&(
            <div style={{marginTop:12,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 14px"}}>
              <div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:4,textTransform:"uppercase"}}>Mensaje original</div>
              <p style={{color:C.muted,fontSize:13,margin:0,lineHeight:1.6}}>{oferta.mensaje}</p>
            </div>
          )}
        </div>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px 20px",marginBottom:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div>
              <div style={{fontWeight:700,color:C.text,fontSize:14}}>Material de clases</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>{soyOfertante?"Subí el material para tu alumno":"Material subido por el docente"}</div>
            </div>
            {soyOfertante&&<button onClick={()=>setShowAdd(v=>!v)} style={{background:C.accentDim,border:"1px solid #F5C84244",borderRadius:8,color:C.accent,padding:"6px 13px",cursor:"pointer",fontSize:12,fontFamily:FONT,fontWeight:700}}>{showAdd?"Cancelar":"+ Agregar"}</button>}
          </div>

          {soyOfertante&&showAdd&&(
            <div style={{background:"#111",border:`1px solid ${C.border}`,borderRadius:12,padding:"14px",marginBottom:14}}>
              <div style={{display:"flex",gap:5,marginBottom:9,flexWrap:"wrap"}}>
                {Object.entries(TIPO_META).map(([v,m])=>(
                  <button key={v} onClick={()=>setNuevoTipo(v)} style={{padding:"5px 10px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:FONT,background:nuevoTipo===v?C.accent:C.surface,color:nuevoTipo===v?"#0D0D0D":C.muted,border:`1px solid ${nuevoTipo===v?"transparent":C.border}`}}>{m.label}</button>
                ))}
              </div>
              <input value={nuevoTitulo} onChange={e=>setNuevoTitulo(e.target.value)} placeholder="Título del material" style={iS}/>
              <textarea value={nuevoBody} onChange={e=>setNuevoBody(e.target.value)}
                placeholder={nuevoTipo==="link"||nuevoTipo==="video"?"URL del enlace/video":"Descripción, texto o instrucciones (opcional)"}
                style={{...iS,minHeight:65,resize:"vertical"}}/>
              <Btn onClick={addContenido} disabled={savingC||!nuevoTitulo.trim()} style={{width:"100%",padding:"8px"}}>{savingC?"Guardando...":"Agregar material"}</Btn>
            </div>
          )}

          {loading?<Spinner/>:contenido.length===0?(
            <div style={{textAlign:"center",padding:"36px 0",color:C.muted}}>
              <div style={{fontSize:22,marginBottom:8,color:C.border}}>◎</div>
              <div style={{fontSize:13}}>{soyOfertante?"Aún no subiste material. Usá + Agregar para empezar.":"El docente aún no subió material."}</div>
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {contenido.map((item,i)=>{
                const m=TIPO_META[item.tipo]||{icon:"·",color:C.text};
                return(
                  <div key={item.id} style={{background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 15px"}}>
                    <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                      <div style={{width:34,height:34,borderRadius:9,background:C.card,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0,color:m.color,fontWeight:700,border:`1px solid ${C.border}`}}>{m.icon}</div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontWeight:600,color:m.color,fontSize:13,marginBottom:2}}>{i+1}. {item.titulo}</div>
                        ...
                        {(item.tipo==="video"||item.tipo==="link"||item.tipo==="archivo")&&item.url&&<a href={item.url} target="_blank" rel="noreferrer" style={{color:C.info,fontSize:12,textDecoration:"none",wordBreak:"break-all"}}>{item.tipo==="video"?"▶ Ver video":item.tipo==="archivo"?"↓ Abrir archivo":"↗ Abrir link"}</a>}
                      </div>
                      {soyOfertante&&<button onClick={()=>removeContenido(item.id)} style={{background:"none",border:"none",color:C.danger,fontSize:15,cursor:"pointer",flexShrink:0,padding:"0 2px"}}>×</button>}
                    </div>
                  </div>
                );
              })}
        ...
        {/* Chat directo entre los dos */}
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden"}}>
          <EspacioChat ofertaId={oferta.id} miEmail={miEmail} otroEmail={otroEmail} otroNombre={otroNombre} session={session}/>
        </div>
      </div>
    </div>
  );
...
// ─── ESPACIO CHAT — chat privado entre docente y alumno de una oferta aceptada ─
function EspacioChat({ofertaId,miEmail,otroEmail,otroNombre,session}){
  ...
  const cargar=useCallback(async()=>{
    try{
      // Usamos getMensajes con publicacion_id=ofertaId como canal privado
      const all=await sb.getMensajes(ofertaId,miEmail,otroEmail,session.access_token);
      setMsgs(all||[]);
      await sb.marcarLeidos(ofertaId,miEmail,session.access_token);
    }catch{}
  },[ofertaId,miEmail,otroEmail,session.access_token]); // eslint-disable-line
  ...
  const enviar=async()=>{
    ...
    setSending(true);
    try{
      await sb.insertMensaje({publicacion_id:ofertaId,de_nombre:miEmail,para_nombre:otroEmail,texto:texto.trim(),leido:false},session.access_token);
  ...
  return(
    <div>
      <div style={{padding:"14px 18px",borderBottom:`1px solid ${C.border}`,fontWeight:700,color:C.text,fontSize:13}}>
        Chat con {otroNombre}
      </div>
      <div style={{height:280,overflowY:"auto",padding:"14px 18px",display:"flex",flexDirection:"column",gap:8}}>
        {loading&&msgs.length===0?<Spinner small/>:msgs.length===0?<div style={{textAlign:"center",color:C.muted,fontSize:12,marginTop:20}}>Empezá la conversación.</div>:null}
        {msgs.map(m=>{
          const mio=m.de_nombre===miEmail;
          return(
            <div key={m.id} style={{display:"flex",justifyContent:mio?"flex-end":"flex-start"}}>
              <div style={{maxWidth:"75%",background:mio?C.accent:C.surface,color:mio?"#0D0D0D":C.text,borderRadius:mio?"14px 14px 4px 14px":"14px 14px 14px 4px",padding:"8px 12px",fontSize:13,lineHeight:1.5}}>
                {m.texto}
                <div style={{fontSize:10,color:mio?"#0D0D0D99":C.muted,marginTop:3,textAlign:"right"}}>{new Date(m.created_at).toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"})}</div>
              </div>
            </div>
          );
        })}
      ...
      <div style={{padding:"12px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8}}>
        <input value={texto} onChange={e=>setTexto(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&enviar()} placeholder="Escribí un mensaje..." style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"9px 13px",color:C.text,fontSize:13,outline:"none",fontFamily:FONT}}/>
        <button onClick={enviar} disabled={sending||!texto.trim()} style={{background:C.accent,border:"none",borderRadius:10,color:"#0D0D0D",padding:"9px 16px",cursor:"pointer",fontWeight:700,fontSize:13,fontFamily:FONT,opacity:(!texto.trim()||sending)?.5:1}}>→</button>
      </div>
    </div>
  );
}
        ...
        <div style={{background:C.card,border:`1px solid #4ECB7133`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {ofertasAceptRec.filter(o=>!o.finalizada_cuenta).map(o=>(
              <div key={o.id} style={{background:C.surface,border:"1px solid #4ECB7133",borderRadius:12,padding:"13px 16px"}}>
      ...
      {(()=>{
        ...
        return(
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>(
                <div key={o.id} style={{background:C.surface,border:`1px solid ${o.estado==="rechazada"?"#E05C5C33":o.estado==="aceptada"?"#4ECB7133":C.border}`,borderRadius:12,padding:"13px 16px"}}>
                  <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:o.estado==="aceptada"?8:0}}>
                    <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0}}>
                      {o.estado==="rechazada"&&<button onClick={()=>descartarOferta(o.id)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.muted,padding:"4px 10px",cursor:"pointer",fontSize:11,fontFamily:FONT}}>Descartar</button>}
                      {o.estado==="aceptada"&&<button onClick={()=>setEspacioModal({...o,_rol:"docente"})} style={{background:C.accent,border:"none",borderRadius:8,color:"#0D0D0D",padding:"6px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>Ver espacio →</button>}
