...
export default function App(){
  ...
  const refreshUnread=useCallback(()=>{
    ...
    Promise.all([
      ...
      // Contar notifs + ofertas aceptadas no vistas
      const vistasKey="ofertasAceptadasVistas_"+session.user.email;
      let vistas=[];try{vistas=JSON.parse(localStorage.getItem(vistasKey)||"[]");}catch{}
      const ofertasAceptadasNuevas=(misOfertas||[]).filter(o=>o.estado==="aceptada"&&!vistas.includes(o.id)).length;
      setNotifCount(nfs.length+ofertasAceptadasNuevas);setNotifs(nfs);
    }).catch(()=>{});
  },[session]);
