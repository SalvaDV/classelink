...
const Avatar=({letra,size=38,img})=>{
  ...
  return<div style={{width:size,height:size,borderRadius:"50%",background:`linear-gradient(135deg,${from},${to})`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:size*0.38,flexShrink:0,boxShadow:"0 2px 8px rgba(0,0,0,.15)",letterSpacing:"-.5px"}}>{(letra||"?").toUpperCase()}</div>;
}
    ...
    <div onClick={()=>onOpenDetail(post)} className="cl-card-anim"
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
          <Tag tipo={post.tipo}/>
          {post.created_at&&(new Date()-new Date(post.created_at))<259200000&&<span style={{background:"#2EC4A0",color:"#fff",borderRadius:20,fontSize:10,fontWeight:700,padding:"2px 7px",letterSpacing:.3}}>NUEVO</span>}
...
function InscribirseBtn({post,session,onDone}){
  ...
  const inscribirse=async()=>{
    ...
    try{
      ...
      sb.sendEmail("nueva_inscripcion",post.autor_email,{pub_titulo:post.titulo,alumno_nombre:sb.getDisplayName(session.user.email)||session.user.email.split("@")[0]},session.access_token).catch(()=>{});
