...
// ─── FAV BUTTON — acepta isFav precargado para evitar N+1 requests en ExplorePage ──
function FavBtn({post,session,onFavChange,isFav:isFavProp,favId:favIdProp}){
  // Si viene isFavProp desde el padre (ExplorePage), usar eso; si no, cargar propio
  const controlled = isFavProp !== undefined;
  const [favId,setFavId]=useState(controlled ? (favIdProp||null) : null);
  const [loading,setLoading]=useState(!controlled);
  useEffect(()=>{
    if(controlled){setFavId(favIdProp||null);setLoading(false);return;}
  ...
  },[post.id,session,isFavProp,favIdProp,controlled]);// eslint-disable-line
  const toggle=async(e)=>{e.stopPropagation();if(loading)return;setLoading(true);
    try{
      if(favId){await sb.deleteFavorito(favId,session.access_token);setFavId(null);}
      else{const r=await sb.insertFavorito({publicacion_id:post.id,usuario_email:session.user.email,usuario_id:session.user.id},session.access_token);setFavId(r[0]?.id);}
  ...
  return(
    <div onClick={()=>onOpenDetail(post)} style={{background:fueRechazado?"#E05C5C08":C.card,border:`1px solid ${fueRechazado?"#E05C5C33":C.border}`,borderRadius:16,padding:"16px 18px",cursor:"pointer",transition:"border-color .2s,transform .15s",position:"relative",overflow:"hidden",fontFamily:FONT}}
      ...
      <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginLeft:"auto"}}><FavBtn post={post} session={session} onFavChange={onFavChange} isFav={isFav} favId={favId}/></div>
      </div>
    </div>
  );
}
...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil}){
  ...
  const clearAll=()=>{setFiltroTipo("all");setFiltroModo("all");setFiltroModalidad("all");setFiltroSinc("all");setFiltroMateria("");setSliderMin(precioMin);setSliderMax(precioMax);setFiltroFechaDesde("");setFiltroFechaHasta("");setFiltroDurMin(0);setBusqueda("");};
...
function DetailModal({post,session,onClose,onChat,onOpenCurso,onOpenPerfil}){
  ...
  useEffect(()=>{
    ...
    sb.incrementarVistas(post.id,session.access_token);
    Promise.all([
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    setSaving(true);setErr("");
    try{
      const data={tipo,materia,titulo,descripcion,autor_id:session.user.id,activo:true,verificado,modo,modalidad:modalidadForm||null};
  ...
  return(
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        <Btn onClick={guardar} disabled={saving||clasesSinc.some(c=>{const p=(t)=>{if(!t)return null;const s=t.split(":");if(s.length<2)return null;const h=parseInt(s[0]);const m=parseInt(s[1]);return isNaN(h)||isNaN(m)?null:h*60+m;};const fi=p(c.hora_inicio);const ff=p(c.hora_fin);return fi!==null&&ff!==null&&ff<=fi;})} style={{width:"100%",padding:"10px",fontSize:13,marginTop:11,borderRadius:11}}>{saving?"Guardando...":editing?"Guardar cambios":"Publicar"}</Btn>
      </div>
    </Modal>
  );
}
