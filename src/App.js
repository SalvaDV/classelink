...
class SafeWrapper extends React.Component {
  ...
  render(){
    if(this.state.crashed){
      const needsTable=this.state.msg?.includes("does not exist")||this.state.msg?.includes("relation")||this.state.msg?.includes("quiz_entregas");
      return(
        <div style={{background:"#E0955C15",border:"1px solid #E0955C33",borderRadius:10,padding:"10px 13px",fontSize:12,color:"#E0955C"}}>
          {needsTable
            ?"⚠ Falta ejecutar el SQL de quiz_entregas en Supabase. Ver instrucciones en la documentación."
            :"⚠ No se pudo cargar el quiz. Recargá la página."}
        </div>
      );
    }
...
function QuizViewer({item,session,esMio,esAyudante,onReabrir}){
  ...
  const {tipo_quiz,preguntas=[],consigna,fecha_inicio,fecha_cierre}=quizData;
...
function CursoPage({post,session,onClose,onUpdatePost}){
  ...
  const setTab=(t)=>{try{sessionStorage.setItem("curso_tab_"+post.id,t);}catch{}setTabActivo(t);};const [nuevoTipo,setNuevoTipo]=useState("video");const [nuevoTitulo,setNuevoTitulo]=useState("");const [nuevoUrl,setNuevoUrl]=useState("");const [nuevoTexto,setNuevoTexto]=useState("");const [savingC,setSavingC]=useState(false);
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px",display:"grid",gridTemplateColumns:hasCal?"1fr 300px":"1fr",gap:22,alignItems:"start"}}>
        <div>
            ...
            {loading?<Spinner/>:contenido.length===0?<div style={{textAlign:"center",padding:"30px 0",color:C.muted,fontSize:13}}>{esMio?"Cargá el primer contenido.":"El docente aún no cargó contenido."}</div>:(
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {contenido.map((c,i)=>{
                  ...
                  if(false&&c.tipo==="quiz___OLD"){
                    return(<div key={c.id} style={{background:C.card,border:`1px solid ${C.purple}33`,borderRadius:12,overflow:"hidden"}}>
                      <div onClick={()=>setExpandedQuizzes(p=>{const n=new Set(p);n.has(c.id)?n.delete(c.id):n.add(c.id);return n;})}
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:7}}>
                          ...
                          <span style={{color:C.muted,fontSize:13,transition:"transform .2s",transform:quizExpanded?"rotate(180deg)":"rotate(0deg)"}}>▾</span>
                        </div>
                      </div>
                      {quizExpanded&&<div style={{padding:"0 15px 12px",borderTop:`1px solid ${C.border}22`}}>
                        ...
                        {!tieneAcceso&&<div style={{fontSize:11,color:C.muted,padding:"8px 0"}}>Inscribite para acceder al quiz.</div>}
                      </div>}
                    </div>);}
          ...
          {tabActivo==="notas"&&<div style={{marginBottom:18}}>
            {(esMio||esAyudante)?(
              contenido.some(c=>c.tipo==="quiz")&&inscripciones.length>0
                ?<TablaNotas contenido={contenido} inscripciones={inscripciones} session={session} publicacionId={post.id}/>
              ...
              <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 18px"}}>
                ...
                {loading?<Spinner small/>:contenido.filter(c=>c.tipo==="quiz").length===0
                  ...
                  :<NotasAlumno contenido={contenido} session={session} publicacionId={post.id}/>
                }
              </div>
            )}
          </div>}
          ...
          {tabActivo==="chat"&&<div style={{marginBottom:18}}>
            {tieneAcceso
              ?<ChatCurso post={post} session={session} ayudantes={post.ayudantes||[]} ayudanteEmails={ayudanteEmails}/>
...
function VerificacionIA({titulo,materia,descripcion,onVerificado,onEstadoChange,token}){
  ...
  const evaluar=async()=>{if(!respuesta.trim())return;setEstado("evaluando");try{const r=await sb.verificarConIA(titulo,materia,descripcion||"",respuesta,token);setFeedback(r.feedback||"");if(r.correcta){setEstado("ok");onVerificado();}else setEstado("error");}catch{setEstado("error");setFeedback("No se pudo evaluar.");}};
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      const data={tipo,materia,titulo,descripcion,autor_id:session.user.id,activo:true,verificado,modo:modoDb,modalidad:modalidadForm||null,nivel:nivel||null,banner_url:bannerUrl||null};
