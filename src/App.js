  ...
  const inner=(
    <div style={{width:224,height:"100%",background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <nav style={{padding:"8px 8px",flex:1,overflowY:"auto"}}>
        ...
        {["salvadordevedia@gmail.com"].includes(session.user.email)&&(
...
function ChatsPage({session,onOpenChat}){
  ...
  const borrarChat=async(pubId,otroEmail,e)=>{
    ...
    try{
      await sb.db(`mensajes?publicacion_id=eq.${pubId}&or=(and(de_nombre.eq.${encodeURIComponent(miEmail)},para_nombre.eq.${encodeURIComponent(otroEmail)}),and(de_nombre.eq.${encodeURIComponent(otroEmail)},para_nombre.eq.${encodeURIComponent(miEmail)}))`,"DELETE",null,session.access_token);
      cargar();
