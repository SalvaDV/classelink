...
function PostCard({post,session,onOpenChat,onOpenDetail,onOpenPerfil,avgPub,countPub,avgUser,yaOferte,fueRechazado,isFav,favId,onFavChange}){
  ...
  return(
    <div onClick={()=>onOpenDetail(post)} className="cl-card-anim" style={{background:fueRechazado?"#E05C5C08":C.card,border:`1px solid ${fueRechazado?"#E05C5C33":C.border}`,borderRadius:16,padding:"16px 18px",cursor:"pointer",transition:"border-color .18s,transform .15s,box-shadow .18s",position:"relative",overflow:"hidden",fontFamily:FONT}}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=fueRechazado?"#E05C5C77":C.accent;e.currentTarget.style.transform="translateY(-2px)";}} onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 4px 20px #0003";}} onMouseLeave={e=>{e.currentTarget.style.borderColor=fueRechazado?"#E05C5C33":C.border;e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none";}}>
  ...
  const buscar=async()=>{
    ...
    try{
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
        ...
        system:`Sos un asistente de búsqueda para ClasseLink, una plataforma educativa argentina.
El usuario describe lo que busca en lenguaje natural. Analizá las publicaciones disponibles y devolvé las más relevantes.
SIEMPRE respondé con JSON válido: {"ids":["id1","id2"],"explanation":"frase breve en español de por qué estas clases son ideales"}
Devolvé máximo 6 IDs ordenados por relevancia. Si no hay nada relevante devolvé {"ids":[],"explanation":"No encontré clases que coincidan con eso."}`,
...
Respondé SOLO JSON.`}]
    ...
    }catch{setResults([]);setExplanation("No pude procesar la búsqueda. Intentá de nuevo.");}
    finally{setLoading(false);}
  };
  return(
