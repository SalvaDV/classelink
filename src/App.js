  ...
  useEffect(()=>{
    Promise.all([
      ...
      Promise.all(ids.map(id=>
        sb.db(`publicaciones_con_autor?id=eq.${id}&select=*`,"GET",null,session.access_token).catch(()=>[])
      )).then(results=>{
      ...
      {loading?<Spinner/>:(
        <>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:18}}>
            ...
            {diaSelec&&clasesEnDia(diaSelec).map((item,i)=>(
              <div key={i} onClick={()=>onOpenCurso(item.post)}
                style={{marginTop:8,background:C.surface,border:`1px solid ${colorPost(item.post)}44`,borderRadius:10,
                  padding:"9px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:3,height:36,background:colorPost(item.post),borderRadius:2,flexShrink:0}}/>
                <div>
                  <div style={{fontWeight:600,color:C.text,fontSize:12}}>{item.post.titulo}</div>
                  <div style={{color:C.muted,fontSize:11}}>{item.clase.hora_inicio} → {item.clase.hora_fin}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px 20px"}}>
            <div style={{fontWeight:700,color:C.text,fontSize:14,marginBottom:12}}>Próximas clases</div>
            {proximas.length===0?(
...
function AsistentePublicacion({tipo,materia,titulo,descripcion,modo,session,onApply}){
  ...
  const generar=async()=>{
    ...
    try{
      const contexto=`Tipo: ${tipo==="oferta"?"Oferta de clases":"Búsqueda de clases"}. Materia: ${materia}. Modalidad: ${modo||"no especificada"}.${titulo?` Título actual: "${titulo}".`:""}${descripcion?` Descripción actual: "${descripcion}".`:""}`;
      const raw=await sb.callIA(
  ...
  return(
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        <div style={{position:"relative",marginBottom:9}}><input value={titulo} onChange={e=>setTitulo(e.target.value.slice(0,80))} placeholder="Título del curso o clase" style={{...iS,marginBottom:0,paddingRight:48}}/><span style={{position:"absolute",right:11,top:"50%",transform:"translateY(-50%)",fontSize:10,color:titulo.length>=70?C.danger:C.muted,fontFamily:FONT,pointerEvents:"none"}}>{titulo.length}/80</span></div>
        ...
        {/* Nivel y requisitos — campos extra */}
        ...
        <div style={{marginBottom:9}}>
          <Label>Requisitos previos (opcional)</Label>
          <input value={requisitos} onChange={e=>setRequisitos(e.target.value.slice(0,150))} placeholder="Ej: Conocimientos básicos de álgebra..." style={{...iS,marginBottom:0}}/>
        </div>
        {tipo==="oferta"&&(<>
          ...
          {/* Verificacion aparece al final, solo cuando hay suficiente contenido */}
          {titulo&&materia&&!verificado&&<VerificacionIA titulo={titulo} materia={materia} descripcion={descripcion} onVerificado={(v)=>{setVerificado(v!==false);setVerificacionPendiente(false);}} onEstadoChange={(e)=>setVerificacionPendiente(e==="cargando")} token={session?.access_token}/>}
          {verificacionPendiente&&<div style={{color:C.warn,fontSize:11,padding:"5px 10px",background:"#E0955C15",borderRadius:7,border:"1px solid #E0955C33",marginTop:5,display:"flex",alignItems:"center",gap:6}}><Spinner small/>Generando pregunta de verificación… (no podés publicar hasta completarla)</div>}
        ...
        <div style={{display:"flex",gap:8,marginTop:11}}>
          ...
          <Btn onClick={guardar} disabled={saving||verificacionPendiente||clasesSinc.some(c=>{const p=(t)=>{if(!t)return null;const s=t.split(":");if(s.length<2)return null;const h=parseInt(s[0]);const m=parseInt(s[1]);return isNaN(h)||isNaN(m)?null:h*60+m;};const fi=p(c.hora_inicio);const ff=p(c.hora_fin);return fi!==null&&ff!==null&&ff<=fi;})} style={{flex:1,padding:"10px",fontSize:13,borderRadius:11}}>{saving?"Guardando...":editing?"Guardar cambios":verificado?"Publicar →":"Publicar sin verificar →"}</Btn>
        </div>
        {showPreview&&titulo&&(
