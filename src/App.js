  ...
  return(
    <div style={{background:C.surface,border:`1px solid ${C.accent}55`,borderRadius:14,padding:16,marginBottom:14,animation:"fadeIn .15s ease"}}>
      ...
      <div style={{display:"flex",gap:7,marginBottom:12}}>
        {[["multiple","📋 Multiple choice"],["entregable","📤 Entregable"]].map(([v,l])=>(
                ...
                return(
                  <button key={d} onClick={()=>setDiaSelec(tieneClase?(selec?null:d):null)}
                    style={{textAlign:"center",padding:"6px 2px",borderRadius:8,fontSize:12,
                      ...
                      position:"relative"}}>
                    {d}
                    {tieneClase&&<div style={{width:4,height:4,borderRadius:"50%",background:selec?"#0D0D0D":C.accent,margin:"2px auto 0"}}/>}
                  </button>
                );
              })}
            </div>
...
function CursoPage({post,session,onClose,onUpdatePost}){
  ...
  return(
    <div  ref={pageRef} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"16px max(16px, calc((100vw - 1100px)/2))",display:"grid",gridTemplateColumns:hasCal&&window.innerWidth>768?"1fr 300px":"1fr",gap:16,alignItems:"start"}}>
        <SafeWrapper>
        <div>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:18}}>
          ...
          {/* ── TAB: Contenido ── */}
          {tabActivo==="contenido"&&<div id="contenido" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 18px",marginBottom:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
    ...
    <Modal onClose={onClose} width="min(660px,97vw)">
      <div style={{padding:"19px 21px"}}>
        ...
        {post.tipo==="oferta"&&(<div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:13}}>
          ...
          :(<><Chip label="MODALIDAD" val="Clase part."/>{post.precio&&<Chip label="PRECIO" val={`${fmtPrice(post.precio)} /${post.precio_tipo||"hora"}`}/>}
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      const data={tipo,materia,titulo,descripcion,autor_id:session.user.id,activo:true,verificado,modo:modoDb,modalidad:modalidadForm||null};// nivel y banner_url omitidos hasta agregar columnas en DB
  ...
  return(
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        {/* Modalidad — chips visuales inmediatamente después del tipo */}
        <div style={{marginBottom:13}}>
          <Label>Modalidad</Label>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
            {[["","Cualquiera","◎",""],["presencial","Presencial","📍","#5CA8E0"],["virtual","Virtual","🌐","#4ECB71"],["mixto","Mixto","⟳","#C85CE0"]].map(([v,l,ic,col])=>(
              <button key={v} type="button" onClick={()=>setModalidadForm(v)}
                style={{padding:"10px 6px",borderRadius:11,fontSize:11,cursor:"pointer",fontFamily:FONT,
                  display:"flex",flexDirection:"column",alignItems:"center",gap:4,
                  background:modalidadForm===v?(col?col+"22":C.accentDim):C.surface,
                  color:modalidadForm===v?(col||C.accent):"#888",
                  border:`1.5px solid ${modalidadForm===v?(col||C.accent)+"88":C.border}`,
                  fontWeight:modalidadForm===v?700:400,transition:"all .15s"}}>
        ...
        <div style={{display:"flex",gap:7,marginBottom:12,flexWrap:"wrap"}}>
          <Label style={{width:"100%",marginBottom:4}}>Nivel de alumnos</Label>
          {[["","Cualquier nivel"],["primaria","Primaria"],["secundaria","Secundaria"],["universitario","Universitario"],["adultos","Adultos"]].map(([v,l])=>(
            <button key={v} type="button" onClick={()=>setNivel(v)}
              style={{padding:"5px 12px",borderRadius:20,fontSize:11,cursor:"pointer",fontFamily:FONT,
                background:nivel===v?C.accent:C.surface,color:nivel===v?"#0D0D0D":C.muted,
                border:`1px solid ${nivel===v?C.accent:C.border}`,fontWeight:nivel===v?700:400}}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div style={{marginBottom:9}}>
          {tipo==="busqueda"?"Requisitos (opcional)":"Requisitos previos (opcional)"}
          <input value={requisitos} onChange={e=>setRequisitos(e.target.value.slice(0,150))} placeholder={tipo==="busqueda"?"Ej: Tengo base de secundaria...":"Ej: Conocimientos básicos de álgebra..."} style={{...iS,marginBottom:0}}/>
        </div>
        {tipo==="oferta"&&(<>
