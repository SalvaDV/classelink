...
// ─── SKILL MANAGER (docente) ──────────────────────────────────────────────────
function SkillManager({post,session}){
  const pubId=post.id;
  ...
  const agregar=()=>{
    ...
    const updated=[...skills,{id:Date.now()+"",nombre:nueva.trim(),peso:1}];
    setSkills(updated);saveSkills(pubId,updated);setNueva("");
  };
  const eliminar=(i)=>{const u=skills.filter((_,idx)=>idx!==i);setSkills(u);saveSkills(pubId,u);};
  const updateNombre=(i,val)=>{const u=skills.map((s,idx)=>idx===i?{...s,nombre:val}:s);setSkills(u);saveSkills(pubId,u);};
  ...
  return(
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 18px",marginBottom:14}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:12}}>
        {skills.map((s,i)=>(
          <div key={s.id} style={{display:"flex",alignItems:"center",gap:8,background:C.surface,borderRadius:9,padding:"7px 11px"}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:SKILL_COLORS[3],flexShrink:0}}/>
            {editIdx===i
              ?<input value={s.nombre} onChange={e=>updateNombre(i,e.target.value)} onBlur={()=>setEditIdx(null)} autoFocus
                  style={{flex:1,background:"transparent",border:"none",color:C.text,fontSize:13,outline:"none",fontFamily:FONT}}/>
      ...
      {skills.length<6&&(
        <div style={{display:"flex",gap:7}}>
          ...
          <button onClick={agregar} disabled={!nueva.trim()}
            style={{background:C.accent,border:"none",borderRadius:8,color:"#0D0D0D",padding:"7px 14px",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:FONT,opacity:!nueva.trim()?0.5:1}}>+</button>
        </div>
      )}
      {skills.length===0&&<div style={{fontSize:12,color:C.muted,textAlign:"center",padding:"8px 0"}}>Agregá las habilidades que los alumnos van a desarrollar.</div>}
    </div>
  );
}
...
function CursoPage({post,session,onClose,onUpdatePost}){
  ...
  const [tabActivo,setTabActivo]=useState(()=>{try{return sessionStorage.getItem("curso_tab_"+post.id)||"contenido";}catch{return "contenido";}});
...
function PostFormModal({session,postToEdit,onClose,onSave}){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      const data={tipo,materia,titulo,descripcion,autor_id:session.user.id,activo:true,verificado,modo:modoDb,modalidad:modalidadForm||null,moneda:moneda||"ARS"};// nivel y banner_url omitidos hasta agregar columnas en DB
