      ...
      <section id="como" style={{background:"#fff",padding:"80px 32px",borderTop:"1px solid #EEF2FF",borderBottom:"1px solid #EEF2FF"}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:32}}>
            {[
              {n:"1",title:"Creá tu cuenta",desc:"Registrarte es gratis y toma menos de un minuto. Solo necesitás un email.",icon:"✉️"},
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"24px 16px"}}>
        <div style={{width:"min(420px,100%)",animation:"fadeSlideUp .5s ease both"}}>
          ...
          <h2 style={{color:"#0D1F3C",fontSize:24,fontWeight:800,margin:"0 0 4px",letterSpacing:"-.4px"}}>
            {mode==="login"?"Bienvenido de nuevo":mode==="register"?"Creá tu cuenta":"Recuperar contraseña"}
          </h2>
          <p style={{color:"#718096",fontSize:14,margin:"0 0 28px"}}>
            {mode==="login"?"Ingresá para continuar":mode==="register"?"Es gratis. Siempre.":"Te enviamos un link a tu email"}
          </p>
...
function DetailModal({post,session,onClose,onChat,onOpenCurso,onOpenPerfil}){
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:200,background:C.bg,display:"flex",flexDirection:"column",fontFamily:FONT,overflowY:"auto",WebkitOverflowScrolling:"touch",animation:"fadeIn .18s ease"}}>
      ...
      {/* ── Barra superior ── */}
      <div style={{position:"sticky",top:0,zIndex:10,background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 20px",height:56,display:"flex",alignItems:"center",gap:12,boxShadow:"0 1px 8px rgba(0,0,0,.06)"}}>
        <button onClick={onClose}
          style={{width:36,height:36,borderRadius:"50%",background:C.bg,border:`1px solid ${C.border}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,color:C.text,flexShrink:0,transition:"background .15s"}}
...
function MiCuentaPage({session,onOpenDetail,onOpenCurso,onEdit,onNew,onOpenChat,onRefreshOfertas,onClearBadge}){
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {/* ── TABS DE NAVEGACIÓN ── */}
      <div className="cl-tabs-scroll" style={{display:"flex",gap:0,marginBottom:16,borderBottom:`2px solid ${C.border}`,background:C.surface,borderRadius:"10px 10px 0 0",padding:"0 2px"}}>
        {[
      ...
      {isMobile&&(
        <>
          ...
          <div style={{position:"fixed",bottom:0,left:0,right:0,background:C.surface,borderTop:`1px solid ${C.border}`,zIndex:50}}>
            <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none",display:"flex",height:58,position:"relative"}}
              className="cl-bottom-nav">
              <style>{`.cl-bottom-nav::-webkit-scrollbar{display:none}.cl-bottom-nav::after{content:'';position:absolute;right:0;top:0;bottom:0;width:32px;background:linear-gradient(to right,transparent,${C.surface});pointer-events:none}`}</style>
              {[
                ...
                <button key={item.id} onClick={()=>setPage(item.id)}
                  style={{flex:"0 0 72px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:"none",border:"none",cursor:"pointer",padding:"6px 0",position:"relative",fontFamily:FONT,borderBottom:`2px solid ${page===item.id?C.accent:"transparent"}`,transition:"border-color .15s"}}>
