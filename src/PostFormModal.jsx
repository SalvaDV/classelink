        ...
        {tipo==="oferta"&&(<>
          ...
          {modo==="particular"&&precio&&(
            <div style={{marginTop:8,borderTop:`1px solid ${C.border}`,paddingTop:10}}>
              <Label>Paquetes de clases <span style={{fontSize:10,color:C.muted,fontWeight:400}}>— descuento por cantidad</span></Label>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {[[5,10],[10,15],[20,20]].map(([cant,desc])=>{
                  const activo=paquetes.some(p=>p.clases===cant);
                  const precioNum=parseFloat(precio)||0;
                  const precioDesc=(precioNum*cant*(1-desc/100)).toFixed(0);
                  return(
                    <div key={cant} onClick={()=>setPaquetes(prev=>activo?prev.filter(p=>p.clases!==cant):[...prev,{clases:cant,descuento:desc,precio_total:parseFloat(precioDesc)}])}
                      style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:10,border:`2px solid ${activo?C.accent:C.border}`,background:activo?C.accentDim:C.bg,cursor:"pointer",transition:"all .15s"}}>
                      <div style={{width:20,height:20,borderRadius:5,border:`2px solid ${activo?C.accent:"#CBD5E0"}`,background:activo?"linear-gradient(135deg,#1A6ED8,#2EC4A0)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                        {activo&&<span style={{color:"#fff",fontSize:11,fontWeight:700}}>✓</span>}
                      </div>
                      <div style={{flex:1}}>
                        <span style={{fontSize:13,fontWeight:600,color:C.text}}>{cant} clases</span>
                        <span style={{fontSize:11,color:C.muted,marginLeft:8}}>{desc}% de descuento</span>
                      </div>
                      {precioNum>0&&<span style={{fontSize:12,fontWeight:700,color:C.success}}>${parseInt(precioDesc).toLocaleString("es-AR")} total</span>}
                    </div>
                  );
                })}
              </div>
