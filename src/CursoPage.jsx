...
function StripeCheckoutBtn({post, session, onDone, onClose}){
  const [estado, setEstado] = useState("idle"); // idle | loading | form | processing | done | error
  ...
  const iniciarPago = async() => {
    ...
    try{
      const moneda = post.moneda?.toLowerCase()==="usd"?"usd":"ars";
      const res = await fetch("https://hptdyehzqfpgtrpuydny.supabase.co/functions/v1/stripe-checkout",{
        method:"POST",
        headers:{"Content-Type":"application/json","apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhwdGR5ZWh6cWZwZ3RycHV5ZG55Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI4MzYyODIsImV4cCI6MjA4ODQxMjI4Mn0.apesTxMiG-WJbhtfpxorLPagiDAnFH826wR0CuZ4y_g","Authorization":`Bearer ${session.access_token}`},
        body: JSON.stringify({
  ...
  // Montar Stripe Elements cuando tenemos client_secret y Stripe cargado
  useEffect(()=>{
    ...
    try{
      ...
      setTimeout(()=>{
        ...
        if(el && cardRef.current) cardRef.current.mount("#stripe-payment-element");
      },100);
  ...
  const confirmarPago = async(e) => {
    ...
    try{
      ...
      // Pago exitoso — inscribir al alumno
  ...
  if(estado==="loading") return(
    <div style={textAlign:"center",padding:"20px 0"}>
      <Spinner/>
      <div style={color:C.muted,fontSize:13,marginTop:8}>Preparando formulario de pago…</div>
    </div>
  );
