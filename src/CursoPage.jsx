      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,width:"min(420px,98vw)",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        {!metodo?(
          <div style={{padding:"0 22px 22px",display:"flex",flexDirection:"column",gap:10}}>
            ...
            {tienePrecio&&post.paquetes&&(()=>{
              let pqs=[];try{pqs=JSON.parse(post.paquetes||"[]");}catch{}
              return pqs.length>0&&(
                <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 16px"}}>
                  <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:.4,marginBottom:10}}>📦 PAQUETES CON DESCUENTO</div>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {pqs.map((pq,i)=>(
                      <button key={i} onClick={()=>inscribirDirecto("paquete_"+pq.clases)}
                        style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderRadius:10,border:`1px solid ${C.success}40`,background:C.success+"08",cursor:"pointer",fontFamily:FONT,transition:"all .15s"}}
                        onMouseEnter={e=>e.currentTarget.style.background=C.success+"15"}
                        onMouseLeave={e=>e.currentTarget.style.background=C.success+"08"}>
                        <div style={{textAlign:"left"}}>
                          <div style={{fontSize:13,fontWeight:700,color:C.text}}>{pq.clases} clases</div>
                          <div style={{fontSize:11,color:C.success,fontWeight:600}}>-{pq.descuento}% de descuento</div>
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontSize:15,fontWeight:800,color:C.success}}>${Number(pq.precio_total||0).toLocaleString("es-AR")}</div>
                          <div style={{fontSize:10,color:C.muted}}>total ({post.moneda||"ARS"})</div>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {tienePrecio&&(post.moneda==="USD"||post.moneda==="EUR")&&(
              <button onClick={()=>setMetodo("stripe")}
                style={{background:"linear-gradient(135deg,#635BFF,#7C3AED)",border:"none",borderRadius:14,color:"#fff",padding:"14px 18px",fontWeight:600,fontSize:14,cursor:"pointer",fontFamily:FONT,display:"flex",alignItems:"center",gap:12,textAlign:"left",boxShadow:"0 4px 14px rgba(99,91,255,.25)"}}>
                <span style={{fontSize:22}}>💳</span>
                <div>
                  <div style={{fontWeight:700}}>Tarjeta de crédito / débito</div>
                  <div style={{fontWeight:400,fontSize:11,opacity:.85}}>Visa, Mastercard, Amex · Pago seguro con Stripe</div>
                </div>
              </button>
            )}
