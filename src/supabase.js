...
/**
 * Crea una preferencia de pago en MP vía Edge Function.
 * Si MP no está habilitado en producción, devuelve { disabled: true }.
 *
 * @param {object} data - { publicacion_id, titulo, descripcion, precio, modo, cantidad, alumno_email, alumno_nombre, docente_email }
 * @param {string} token - JWT del usuario
 * @returns {{ checkout_url, preference_id, expires_at } | { disabled: true }}
 */
export const createMPCheckout = async (data, token) => {
  ...
  // La función devuelve 503 cuando MP_ENABLED=false — no es un error real
  if (res.status === 503 && json.code === "MP_DISABLED") {
    return { disabled: true };
  }

  if (!res.ok) throw new Error(json.error ?? `Error ${res.status}`);
  return json; // { checkout_url, preference_id, expires_at }
};
...
/**
 * Obtiene los pagos recibidos para las publicaciones de un docente.
 */
