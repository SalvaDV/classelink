...
// ── IA helper — centraliza todas las llamadas a Anthropic ────────────────────
// La API key se puede configurar con sb.setAnthropicKey(key) o via
// la variable de entorno REACT_APP_ANTHROPIC_KEY
let _anthropicKey = "";
try { _anthropicKey = process.env.REACT_APP_ANTHROPIC_KEY || ""; } catch {}
export const setAnthropicKey = (k) => { _anthropicKey = k; };

export const callIA = async (system, userMsg, maxTokens=600) => {
  const headers = { "Content-Type": "application/json" };
  if (_anthropicKey) headers["x-api-key"] = _anthropicKey;
...
Formato: {"pregunta":"...","correcta":true,"feedback":"..."}`;
  ...
  const raw = await callIA(system, userMsg, 400);
