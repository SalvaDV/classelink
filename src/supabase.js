...
// ── Google OAuth ──────────────────────────────────────────────────────────────
// Redirige al provider de Google. Supabase maneja el callback y vuelve
// a window.location.origin con el access_token en el hash de la URL.
export const signInWithGoogle = () => {
  const redirectTo = `${window.location.origin}`;
  window.location.href = `${SUPABASE_URL}/auth/v1/authorize?provider=google&redirect_to=${encodeURIComponent(redirectTo)}`;
};

// Llama a esto en App.js al montar (useEffect) para detectar el callback de Google.
// Retorna { access_token, refresh_token, user } o null si no hay hash OAuth.
export const getSessionFromUrl = async () => {
  ...
  // Obtener los datos del usuario con el token recibido
...
export const getUsuarioByIdFull = (id,token) =>
  db(`usuarios?id=eq.${id}&select=id,email,nombre,display_name,bio,ubicacion,avatar_url`,"GET",null,token).then(r=>r?.[0]||null).catch(()=>null);
...
export const getUsuarioByEmail = (email,token) =>
  db(`usuarios?email=eq.${encodeURIComponent(email)}&select=id,email,nombre,display_name,bio,ubicacion,avatar_url`,"GET",null,token).then(r=>r?.[0]||null).catch(()=>null);
...
/**
 * Envía un email transaccional usando un template predefinido.
 *
 * Templates disponibles:
 *   bienvenida         → { nombre, email }
 *   nueva_inscripcion  → { pub_titulo, alumno_nombre }
 *   oferta_recibida    → { pub_titulo, docente_nombre, mensaje? }
 *   oferta_aceptada    → { pub_titulo, alumno_nombre }
 *   pago_aprobado      → { pub_titulo, monto, mp_payment_id }
 *   clase_finalizada   → { pub_titulo, docente_nombre }
 *   nuevo_mensaje      → { pub_titulo, de_nombre, preview? }
 *
 * @param {string} template - nombre del template
 * @param {string|string[]} to - destinatario(s)
 * @param {object} data - datos para el template
 * @param {string} token - JWT del usuario (opcional)
 */
  ...
  try {
    ...
    for (const alerta of alertas) {
      try {
        ...
        // ── Filtro por tipo de publicación (nuevo campo) ──
        const tipoAlerta = alerta.tipo_alerta || "ambos";
        if (tipoAlerta !== "ambos" && pub.tipo !== tipoAlerta) continue;

        // ── Scoring con matching semántico mejorado ────────────────────────────
        let score = 0;

        // Materia exacta = bonus grande
        ...
        if (score < 2 && alerta.descripcion && pub.titulo) {
          try {
            const raw = await callIA(
              ...
              `Búsqueda del usuario: "${alerta.descripcion}"
Publicación: "${pub.titulo} — ${(pub.descripcion||"").slice(0,100)}"`,
              50, token
            );
